...
description: Use when creating or modifying ShipChamber shared UI data access, OpenCode SDK calls, `RuntimeAPIs`, runtime fetch/auth/URLs, authenticated browser assets, bridges/proxies, runtime switching, or server API routes.
---
...
- ShipChamber-owned HTTP capabilities use `RuntimeAPIs` where runtime-specific behavior exists, otherwise explicit ShipChamber routes through `runtimeFetch`.
...
| ShipChamber HTTP route | `runtimeFetch('/api/...')` |
...
2. **Keep ShipChamber routes explicit.** Register them before the generic OpenCode proxy.
...
Plain `fetch` is reserved for intentional external origins that are not the active ShipChamber/OpenCode runtime.
...
- ShipChamber routes win before generic proxy fallback.
