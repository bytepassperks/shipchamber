...
- **VS Code: comments on code.** Select lines, click the `+` in the gutter or right-click → ShipChamber → Add Comment, and write your note. It stays pinned to the code and goes out with your next message as a context card. Works in diffs too (thanks to @felipegenef).
...
- Server: `SHIPCHAMBER_CHATS_DIR` moves the folder that holds chats without a project, for setups where OpenCode runs as a different user (thanks to @steffenmaechtel).
- CLI: on Linux, `shipchamber startup enable` warns when the service would stop at logout and shows the `loginctl` command that keeps it running (thanks to @IbrahimKhan12).
...
- CLI: on Windows, `shipchamber` starts the server under Bun when Bun is installed.
...
- **Message queue:** messages you queue while a session is busy are now sent by the ShipChamber server. They go out even if you close the tab that queued them, and every device shows the same queue.
...
- **OpenCode Go:** every request ShipChamber sends on its own, such as commit messages, pull request text, recaps and follow-ups, walkthroughs, Goal Mode checks, notes, and usage, now carries the `x-opencode-session` header OpenCode Go requires from 6 September. Chat traffic already had it. Update before that date if you use OpenCode Go.
...
- Settings: new installs start on the ShipChamber light and dark themes. An existing Flexoki choice is kept.
...
- **Linear:** connect a workspace in Settings → Integrations, browse and filter issues, and start a session or worktree from an issue. ShipChamber reports session progress back to Linear and can attach an issue to your next message (thanks to @AlexKutas).
...
- Settings: each ShipChamber instance remembers its own theme, so windows connected to different instances keep the look you gave them (thanks to @kydorn).
...
- Scheduled tasks: Goal, Auto-accept, and the other task settings survive when an older ShipChamber build shares the same project config.
...
- **Worktrees:** a session menu can move an idle session and its sub-sessions into an existing worktree. ShipChamber finds worktrees created elsewhere, asks before carrying uncommitted changes over, and keeps those changes safe if the move fails partway (thanks to @mattv8).
...
- Web/PWA: the installed app uses the shorter name "ShipChamber" (thanks to @bketelsen and @greghaynes).
...
- Updates: "Update OpenCode" no longer fails with a bare "Bad Request". ShipChamber names the release it will install and shows OpenCode's reason when it refuses (thanks to @mdatsev and @yulia-ivashko).
...
- Desktop: a slow or interactive shell startup file no longer stalls startup while ShipChamber looks for OpenCode. Each probe gives up after five seconds, which is what left a Homebrew OpenCode looking undetected from a Dock launch (thanks to @mskadu).
...
- Server: `shipchamber connect-url` no longer risks tearing `settings.json` while the desktop app runs, which could unpair every device (thanks to @shijie152).
...
- Devices: re-pairing a phone keeps the name you gave it instead of resetting it to "ShipChamber Mobile".
...
- Auth: an expired ShipChamber login is announced within seconds by a banner with a Log in button. Sending pauses until you log in, and a conversation that failed to load reloads itself afterwards.
...
- Desktop: notification clicks and shipchamber:// links open in one window instead of all of them.
...
- Relay: paired devices are no longer logged out when the app restarts while another local ShipChamber process is running.
...
- Diff: the context-panel diff can show every change on the current branch against its base branch. ShipChamber detects the base when Git knows it, and asks you once when it does not.
...
- **Agent browser control:** agents can open a page and work with it. They read what is on screen, click, type, scroll, look at how an element renders, switch between mobile, tablet and desktop layouts, and save a screenshot into the project, so they can check their own work. Turn it on or off in Settings → General → ShipChamber Tools.
...
- Browser: when ShipChamber runs on another machine, the desktop app opens its dev servers through a local port, so pages load with working hot reload and developer tools, and links to another local port stay on that machine. In a web browser tab you can open only dev servers on your own machine.
...
- Scheduled Tasks: when two ShipChamber servers share a project configuration, a scheduled occurrence runs once; both servers no longer start duplicate sessions (thanks to @makeittech).
...
- Mobile/Android: pairing QR codes work in older WebViews that misread `shipchamber://` links (thanks to @CMBill).
...
- Sessions: starting ShipChamber from a folder that is not your project, your home folder for instance, no longer floods you with "not a git repository" errors that could stop sessions and projects from loading (thanks to @makeittech).
...
- UI: ShipChamber, in dark and light, is the new default theme.
...
- Desktop: bundled OpenCode updates together with ShipChamber, with no separate update to accept (thanks to @yulia-ivashko).
...
- **Agents:** through the ShipChamber tool, an agent on a managed local instance can create, send to, fork, inspect, and wait for sessions, make an isolated worktree, and manage scheduled tasks. A new Schedule a Task starter walks you through setting one up from chat.
...
- Settings: every page uses the same responsive layout, and navigation is grouped into ShipChamber, Workspace, OpenCode, and Library.
...
- VS Code: per-session permission auto-accept works again, survives an extension restart, and covers subagent sessions while an ShipChamber view is open.
...
- **Session goals:** arm the target button in the composer and your next prompt becomes a [goal](https://docs.shipchamber.com/session-goals/). The session keeps working on it by itself, and a small model audits every finished turn, until the goal is done, blocked, or past its optional token budget.
...
- Remote access: running several ShipChamber instances on one machine no longer lands paired devices on a random one, which was behind the intermittent "Unable to reach server" errors on paired phones.
...
- **Remote access:** the new [private relay](https://docs.shipchamber.com/private-relay/) reaches your instance from anywhere over an end-to-end-encrypted tunnel, with no open ports and no third-party tunnel. It switches on when you pair a device over it and switches off when no paired device uses it (thanks to @yulia-ivashko).
- **Mobile:** the iOS and Android apps are open for testing. Join the [iOS beta on TestFlight](https://testflight.apple.com/join/5ek6GU1E) or take the Android APK from the [latest release](https://github.com/bytepassperks/shipchamber/releases/latest). Scan the QR code from "Add a device" on your server to connect.
...
- **Pairing:** the redesigned ["Add a device"](https://docs.shipchamber.com/connect-devices/) dialog asks where the device will be used, Anywhere, Home network only, or This computer only, then shows a big QR code with a copyable link and closes itself once the device connects (thanks to @yulia-ivashko).
...
- Mobile: the app checks for ShipChamber updates itself, and Android shows a download toast.
...
- Desktop: a remote ShipChamber instance can carry custom headers, which is what Cloudflare Access-style setups need. Settings and environment variables still win over them.
...
- **OpenCode: your own OpenCode is left alone.** When one is already running, whether the TUI, `opencode serve`, or a daemon on port 4096, the app starts its own server. This clears the "ShipChamber could not finish initialization" error and stops the app from opening or closing your OpenCode as it starts and quits.
...
- **CLI: `shipchamber update` works again.** A missing helper had broken the command.
...
- CLI: `shipchamber update` works again.
...
- GitHub: ShipChamber finds the upstream of a forked repository again.
...
- CLI: startup and status commands check the port before treating a running process as your ShipChamber server.
...
- CLI: a leftover server PID file no longer makes the CLI treat an unrelated process as ShipChamber.
...
- Sessions: new draft sessions now start from the default model and agent instead of inheriting the previous session's selection, and fall back to OpenCode's own `default_agent` (and its model) when no ShipChamber default is set.
...
- Web/Remote Instances: added a headless web app mode, and remote instance switching now changes the ShipChamber API endpoint without loading the full remote UI.
...
- Desktop: added optional password setting in ShipChamber sessions settings for the local Desktop server.
...
- CLI: fixed a startup regression in global npm/bun installs where wrapper or symlinked `shipchamber` entrypoints could exit without output on commands like `--version` or `status`.
...
- Desktop: switch between local and remote ShipChamber instances, plus a thinner runtime.
...
- UI: introduced a token-based theming system, 18 themes with light/dark variants, and custom user themes from `~/.config/shipchamber/themes`.
...
- Added queue mode toggle to ShipChamber settings (chat section) with persistence across runtimes.
...
- Added ShipChamber VS Code extension with editor integration: file picker, click-to-open in tool parts.
