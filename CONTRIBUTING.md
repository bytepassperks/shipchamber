# Contributing to ShipChamber

## Getting Started

```bash
git clone https://github.com/bytepassperks/shipchamber.git
cd shipchamber
bun install
```
...
Both are configurable via env vars: `SHIPCHAMBER_PORT`, `SHIPCHAMBER_HMR_UI_PORT`, `SHIPCHAMBER_HMR_API_PORT`.
...
`bun run vscode:dev` opens an Extension Development Host automatically. You can override the editor or workspace with `SHIPCHAMBER_VSCODE_BIN` and `SHIPCHAMBER_VSCODE_DEV_WORKSPACE`.

Example: `SHIPCHAMBER_VSCODE_BIN=cursor bun run vscode:dev`.
...
| `bun run pack:web` | Create a package archive for `@shipchamber/web` |
...
```bash
SHIPCHAMBER_TARGET_ARCH=x64 bun run electron:build
# On an arm64 host:
SHIPCHAMBER_TARGET_ARCH=arm64 bun run electron:build
...
Open an [issue](https://github.com/bytepassperks/shipchamber/issues) or ask in [Discord](https://discord.gg/ZYRSdnwwKA).
