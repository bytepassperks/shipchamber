...
const localOrigin = readArgValue('--shipchamber-local-origin');
const apiBaseUrl = readArgValue('--shipchamber-api-base-url');
const clientToken = readArgValue('--shipchamber-client-token');
const runtimeHeadersRaw = readArgValue('--shipchamber-runtime-headers');
const homeDirectory = readArgValue('--shipchamber-home');
const macosMajorRaw = readArgValue('--shipchamber-macos-major');
const macosMajor = Number.parseInt(macosMajorRaw, 10);
const trayEnabled = process.platform !== 'darwin' || readArgValue('--shipchamber-tray-enabled') !== '0';
...
//  - __SHIPCHAMBER_ELECTRON__ is a shell-identity flag (no capability).
...
//  - __SHIPCHAMBER_DESKTOP__ is the IPC channel to the main process. It is
...
const isLocalPage = currentOrigin !== 'null'
  && (currentOrigin === 'shipchamber-ui://app'
  || (localOrigin && currentOrigin === localOrigin));

// Remote pages need __SHIPCHAMBER_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__SHIPCHAMBER_LOCAL_ORIGIN__', localOrigin);
}

if (apiBaseUrl) {
  contextBridge.exposeInMainWorld('__SHIPCHAMBER_API_BASE_URL__', apiBaseUrl);
}

if (clientToken && isLocalPage) {
  contextBridge.exposeInMainWorld('__SHIPCHAMBER_CLIENT_TOKEN__', clientToken);
}
...
const relayHostId = readArgValue('--shipchamber-relay-host-id');
if (relayHostId && isLocalPage) {
  contextBridge.exposeInMainWorld('__SHIPCHAMBER_RELAY_HOST_ID__', relayHostId);
}
...
if (runtimeHeadersRaw && isLocalPage) {
  try {
    ...
    if (runtimeHeaders && typeof runtimeHeaders === 'object') {
      contextBridge.exposeInMainWorld('__SHIPCHAMBER_RUNTIME_HEADERS__', runtimeHeaders);
    }
  } catch {
  }
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__SHIPCHAMBER_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__SHIPCHAMBER_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__SHIPCHAMBER_ELECTRON__', {
  runtime: 'electron',
  arch: process.arch,
  trayEnabled,
});

contextBridge.exposeInMainWorld('__SHIPCHAMBER_PLATFORM__', process.platform);
...
ipcRenderer.on('shipchamber:emit', (_evt, payload) => {
...
ipcRenderer.on('shipchamber:relay-dev-tunnel-connect', (event, payload) => {
...
// ipcMain.handle('shipchamber:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
const desktopBridge = {
  invoke: (cmd, args) => ipcRenderer.invoke('shipchamber:invoke', cmd, args || {}),
  openDialog: (options) => ipcRenderer.invoke('shipchamber:dialog:open', options || {}),
  grantFileAccess: (filePath) => ipcRenderer.invoke('shipchamber:file:grant-existing', filePath),
  openExternal: (url) => ipcRenderer.invoke('shipchamber:invoke', 'desktop_open_external_url', { url }),
...
contextBridge.exposeInMainWorld('__SHIPCHAMBER_DESKTOP__', desktopBridge);
