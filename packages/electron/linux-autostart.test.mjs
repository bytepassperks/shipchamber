...
test('prefers APPIMAGE path for Linux autostart Exec', () => {
  assert.equal(
    resolveLinuxLaunchExecutable({
      env: { APPIMAGE: '/home/user/ShipChamber.AppImage' },
      execPath: '/tmp/.mount_OpenChXXXX/shipchamber',
    }),
    '/home/user/ShipChamber.AppImage',
  );
});
...
test('writes and removes the XDG autostart file', async () => {
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'shipchamber-autostart-'));
  ...
  try {
    ...
    const enabled = await setLinuxAutostartEnabled({
      enabled: true,
      backgroundArg: '--background',
      env: { ...env, APPIMAGE: '/opt/ShipChamber.AppImage' },
      homeDir,
    });
    ...
    assert.match(contents, /Exec=\/opt\/ShipChamber\.AppImage --background/);
