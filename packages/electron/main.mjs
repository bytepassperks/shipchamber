...
import { mintOutsideFileGrant } from '@shipchamber/web/server/lib/fs/routes.js';
import { fetchUpdateNotes } from '@shipchamber/web/server/lib/changelog/update-notes.js';

const execFileAsync = promisify(execFile);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const isDev = process.env.SHIPCHAMBER_ELECTRON_DEV === '1' || !app.isPackaged;
const electronStartupStartedAt = performance.now();

const DEEP_LINK_PROTOCOL = 'shipchamber';
const UI_PROTOCOL = 'shipchamber-ui';
const PACKAGED_APP_USER_MODEL_ID = 'dev.shipchamber.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.shipchamber.desktop.dev';
...
// ~/Library/Logs/ShipChamber/ (not ~/Library/Logs/@shipchamber/electron/).
app.setName('ShipChamber');
if (process.platform === 'linux') {
  app.setDesktopName('shipchamber.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'ShipChamber Dev'));
}
...
if (shouldIgnoreLoopbackConnectionLimit({
  development: isDev,
  packagedUi: process.env.SHIPCHAMBER_ELECTRON_USE_BUNDLED_UI === '1',
})) {
...
// that output never lands in ~/Library/Logs/ShipChamber/main.log and we
...
const recordElectronStartupPerformance = (phase, details = {}) => {
  const enabled = STARTUP_PERF_ENABLED_VALUES.has(String(process.env.SHIPCHAMBER_STARTUP_PERF ?? '').toLowerCase());
...
const readAppMetadata = () => {
  ...
  return { name: '@shipchamber/electron', version: app.getVersion() };
};
...
const GITHUB_BUG_REPORT_URL = 'https://github.com/bytepassperks/shipchamber/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/bytepassperks/shipchamber/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `ShipChamber detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const requestQuitWithConfirmation = async () => {
  ...
  try {
    const result = await dialog.showMessageBox({
      type: 'warning',
      title: 'Quit ShipChamber?',
      message: 'Quit ShipChamber?',
...
const refreshQuitRiskFlags = async () => {
  ...
  const scheduledUrl = `${base}/api/shipchamber/scheduled-tasks/status`;
  const tunnelUrl = `${base}/api/shipchamber/tunnel/status`;
...
const settingsFilePath = () => {
  if (typeof process.env.SHIPCHAMBER_DATA_DIR === 'string' && process.env.SHIPCHAMBER_DATA_DIR.trim()) {
    return path.join(process.env.SHIPCHAMBER_DATA_DIR.trim(), 'settings.json');
  }
  return path.join(os.homedir(), '.config', 'shipchamber', 'settings.json');
};
...
// session expires) reuses the same "ShipChamber Desktop" record instead of
...
const shouldUsePackagedUi = () => {
  if (process.env.SHIPCHAMBER_ELECTRON_LOAD_SERVER_UI === '1') return false;
  if (process.env.SHIPCHAMBER_ELECTRON_USE_BUNDLED_UI === '1') return true;
...
const injectRuntimeConfigIntoHtml = (html) => {
  ...
  const initScript = `<script>if(window.__SHIPCHAMBER_LOCAL_ORIGIN__===undefined){window.__SHIPCHAMBER_LOCAL_ORIGIN__=${JSON.stringify(localOrigin)};}if(window.__SHIPCHAMBER_API_BASE_URL__===undefined){window.__SHIPCHAMBER_API_BASE_URL__=${JSON.stringify(apiBaseUrl)};}if(window.__SHIPCHAMBER_CLIENT_TOKEN__===undefined&&${JSON.stringify(state.clientToken || '')}){window.__SHIPCHAMBER_CLIENT_TOKEN__=${JSON.stringify(state.clientToken || '')};}</script>`;
...
/**
 * The browser panel's own session, kept separate from ShipChamber's.
 *
 ...
 * own storage.
 */
const BROWSER_PANEL_PARTITION = 'persist:shipchamber-browser';
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToPrimaryWindow('shipchamber:open-session', { sessionId, directory });
    }
    release();
  });
...
import { pathLooksUserConfigured, mergePathValues } from '@shipchamber/web/server/lib/opencode/path-utils.js';
import { clearAppImageArgv0FromProcessEnv } from '@shipchamber/web/server/lib/inherited-env.js';
...
const shouldSkipLocalServer = () => {
  inheritUserShellEnv();
  return process.env.SHIPCHAMBER_SKIP_LOCAL_SERVER === '1';
};
...
const spawnLocalServer = async () => {
  ...
  // The server module reads ENV_DESKTOP_NOTIFY / SHIPCHAMBER_DIST_DIR /
  // SHIPCHAMBER_RUNTIME at import time (top-level const), so these must be
  ...
  process.env.SHIPCHAMBER_HOST = bindHost;
  process.env.SHIPCHAMBER_DESKTOP_LAN_ACCESS_ACTIVE = effectiveLanAccessEnabled ? 'true' : 'false';
  if (lanAccessBlockedByMissingPassword) {
    process.env.SHIPCHAMBER_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
  } else {
    delete process.env.SHIPCHAMBER_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
  }
  process.env.SHIPCHAMBER_DIST_DIR = resolveWebDistDir();
  ...
  process.env.SHIPCHAMBER_OPENCODE_CWD = resolveManagedOpenCodeCwd({
    env: process.env,
    homedir: () => os.homedir(),
  });
  process.env.SHIPCHAMBER_DESKTOP_NOTIFY = 'true';
  if (desktopUiPassword) {
    process.env.SHIPCHAMBER_UI_PASSWORD = desktopUiPassword;
  } else {
    delete process.env.SHIPCHAMBER_UI_PASSWORD;
  }
  process.env.SHIPCHAMBER_SKIP_API_COMPRESSION = process.env.SHIPCHAMBER_SKIP_API_COMPRESSION || 'true';
  process.env.NO_PROXY = process.env.NO_PROXY || 'localhost,127.0.0.1';
  process.env.no_proxy = process.env.no_proxy || 'localhost,127.0.0.1';

  const { startWebUiServer } = await import('@shipchamber/web/server/index.js');
...
`;
  ...
  const child = spawn('/bin/sh', ['-c', script, 'shipchamber-opencode-killer', normalizedPid, normalizedPort, String(OPENCODE_SHUTDOWN_GRACE_MS / 1000)], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  });
...
const buildInitScript = (localOrigin, bootOutcome, apiBaseUrl = '', clientToken = '', requestHeaders = {}) => {
  ...
  return [
    '(function(){',
    `try{var __oc_local=${local};var __oc_api=${apiBase};var __oc_headers=${headers};var __oc_packaged=${packagedOrigin};var __oc_origin=window.location&&window.location.origin||'';var __oc_is_packaged=__oc_origin===__oc_packaged;var __oc_is_local=__oc_local&&__oc_origin===new URL(__oc_local).origin;window.__SHIPCHAMBER_MACOS_MAJOR__=${macVersion};window.__SHIPCHAMBER_LOCAL_ORIGIN__=__oc_local;window.__SHIPCHAMBER_API_BASE_URL__=__oc_api;if(__oc_is_local||__oc_is_packaged){window.__SHIPCHAMBER_HOME__=${home};window.__SHIPCHAMBER_RUNTIME_HEADERS__=__oc_headers;}if((__oc_is_local||__oc_is_packaged)&&${token}){window.__SHIPCHAMBER_CLIENT_TOKEN__=${token};}var __oc_bo=${outcome};if(__oc_bo){window.__SHIPCHAMBER_DESKTOP_BOOT_OUTCOME__=__oc_bo;}}catch(_e){}`,
    '}())',
  ].join('');
};
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ShipChamber loading icon">
...
const loginRemoteAndIssueClientToken = async ({ url, password, trustDevice, requestHeaders }) => {
  ...
  const loginResponse = await fetch(new URL('/auth/session', `${baseUrl}/`).toString(), {
    ...
    body: JSON.stringify({
      ...
      clientLabel: 'ShipChamber Desktop',
      ...clientIdentity,
    }),
  });
  if (!loginResponse.ok) {
...
const emitToWindow = (browserWindow, event, detail) => {
  if (!browserWindow || browserWindow.isDestroyed()) return;
  browserWindow.webContents.send('shipchamber:emit', { event, detail });
};
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'ShipChamber',
...
const redeemConnectPairingDeepLink = async (payload, serverUrl) => {
  const response = await requestJsonWithTimeout(`${serverUrl.replace(/\/+$/g, '')}/api/client-auth/pairing/redeem`, {
    ...
    body: JSON.stringify({
      ...
      clientLabel: 'ShipChamber Desktop',
      clientKind: 'desktop',
      deviceName: 'ShipChamber Desktop',
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to ShipChamber server?',
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'focus') {
    ...
    emitToAllWindows('shipchamber:deep-link-focus', { reason: link.value || null });
    return;
  }

  if (link.type === 'session' && link.value) {
    emitToPrimaryWindow('shipchamber:open-session', { sessionId: link.value });
    return;
  }
...
const dispatchMenuAction = (action) => {
  ...
  emitToWindow(target, 'shipchamber:menu-action', action);
  dispatchDomEventToWindow(target, 'shipchamber:menu-action', action);
};
...
const dispatchAddSelectionToChat = () => {
  const target = getMenuTargetWindow();
  if (target) emitToWindow(target, 'shipchamber:menu-action', 'add-selection-to-chat');
};
...
const dispatchOpenMiniChat = (browserWindow) => {
  ...
  if (target) emitToWindow(target, 'shipchamber:open-mini-chat');
};

const dispatchCheckForUpdates = () => {
  emitToAllWindows('shipchamber:check-for-updates');
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    dispatchDomEventToWindow(browserWindow, 'shipchamber:check-for-updates');
  }
};
...
const createBrowserWindow = ({ label, restoreGeometry, url, runtimeConfig = {} }) => {
  ...
  const options = {
    ...
    webPreferences: {
      additionalArguments: [
        `--shipchamber-local-origin=${desktopLocalOrigin}`,
        `--shipchamber-api-base-url=${desktopApiBaseUrl}`,
        `--shipchamber-client-token=${desktopClientToken}`,
        `--shipchamber-runtime-headers=${JSON.stringify(desktopRequestHeaders)}`,
        `--shipchamber-home=${desktopHome}`,
        `--shipchamber-macos-major=${desktopMacosMajor}`,
        `--shipchamber-tray-enabled=${trayEnabled ? '1' : '0'}`,
        `--shipchamber-boot-outcome=${JSON.stringify(state.bootOutcome || null)}`,
        `--shipchamber-relay-host-id=${rendererRuntimeConfig.relayHostId || ''}`,
      ],
  ...
  browserWindow.on('resize', () => {
    if (process.platform === 'darwin') {
      emitToWindow(browserWindow, 'shipchamber:window-resized');
    }
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('maximize', () => {
    emitToWindow(browserWindow, 'shipchamber:window-maximized-changed', { maximized: true });
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('unmaximize', () => {
    emitToWindow(browserWindow, 'shipchamber:window-maximized-changed', { maximized: false });
    debounceWindowStatePersist(browserWindow, false);
  });
...
const resolveInitialUrl = async () => {
  const hmrApiPort = process.env.SHIPCHAMBER_HMR_API_PORT || '3901';
  const hmrUiPort = process.env.SHIPCHAMBER_HMR_UI_PORT || '5173';
  ...
  const envTarget = normalizeHostUrl(process.env.SHIPCHAMBER_SERVER_URL || '');
  const config = readDesktopHostsConfig();
  if (envTarget) {
  ...
  if (!initialUrl) {
    throw new Error(
      'SHIPCHAMBER_SKIP_LOCAL_SERVER=1 requires bundled UI, a running desktop HMR UI, or a reachable remote instance.',
    );
  }
...
const setupAutoUpdater = () => {
  ...
  autoUpdater.on('download-progress', (progress) => {
    ...
    emitToAllWindows('shipchamber:update-progress', mapUpdaterProgressEvent({
      event: 'Progress',
      data: {
...
const iconToDataUrl = async (iconPath, appName) => {
  ...
  const tempPath = path.join(os.tmpdir(), `shipchamber-icon-${safeName}-${Date.now()}.png`);
  try {
...
const getDevTunnelClient = async () => {
  if (!devTunnelClientPromise) {
    devTunnelClientPromise = import('@shipchamber/web/server/lib/dev-tunnel/client.js')
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    // on the remote ShipChamber host, so the browser panel loads a real origin
     ...
     *
     * nativeTheme.themeSource is app-wide and would drag ShipChamber's own
     ...
     * to third-party hosts from ShipChamber itself. The bytes come back as a
    ...
    // touch ShipChamber's session or any other window's storage.
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/shipchamber/credentials'];
    ...
    case 'desktop_get_installed_apps': {
      ...
      const refresh = async () => {
        ...
        emitToAllWindows('shipchamber:installed-apps-updated', apps);
      };
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires ShipChamber.app to be installed in /Applications');
          }
        } catch (error) {
    ...
    case 'desktop_focus_main_window': {
      ...
      if (sessionId) {
        emitToWindow(state.mainWindow, 'shipchamber:open-session', { sessionId, directory });
      } else if (mode === 'draft') {
        emitToWindow(state.mainWindow, 'shipchamber:open-draft-session', { directory, projectId });
      }
      return { focused: true };
    }
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About ShipChamber', click: () => dispatchAction('about') },
        {
...
const isLocalSender = (webContents) => {
  try {
    ...
    if (isDev && url.origin === `http://127.0.0.1:${process.env.SHIPCHAMBER_HMR_UI_PORT || '5173'}`) return true;
    if (url.protocol !== 'http:' && url.protocol !== 'https:') return false;
    if (state.localOrigin) {
      try {
...
ipcMain.handle('shipchamber:invoke', async (event, command, args) => {
...
ipcMain.handle('shipchamber:dialog:open', async (event, options) => {
...
ipcMain.handle('shipchamber:file:grant-existing', async (event, filePath) => {
  if (!isLocalSender(event.sender)) {
...
const focusMainWindowWithSession = async (sessionId, directory) => {
  if (state.mainWindow && !state.mainWindow.isDestroyed()) {
    ...
    state.mainWindow.focus();
    if (sessionId) {
      emitToWindow(state.mainWindow, 'shipchamber:open-session', { sessionId, directory: directory || '' });
    }
    return;
  }
  if (sessionId) pendingDeepLinks.push({ type: 'session', value: sessionId });
...
const dispatchTrayAction = async (action) => {
  ...
  if (action.type === 'respond-permission') {
    const target = (state.mainWindow && !state.mainWindow.isDestroyed())
      ? state.mainWindow
      : await revealMainWindow();
    emitToWindow(target, 'shipchamber:tray-action', action);
    return;
  }
  ...
  if (action.type === 'focus-session') {
    ...
    if (surface && surface.__ocMiniChat === true && action.sessionId) {
      ...
      surface.show();
      surface.focus();
      emitToWindow(surface, 'shipchamber:open-session', {
  ...
  if (action.type === 'new-session') {
    emitToWindow(target, 'shipchamber:open-draft-session', { directory: '', projectId: '' });
  }
...
app.whenReady().then(async () => {
  ...
  powerMonitor.on('resume', () => {
    emitToAllWindows('shipchamber:system-resume', { timestamp: Date.now() });
  });
}).catch((error) => {
