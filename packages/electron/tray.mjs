...
//
// Surfaces a glanceable, always-visible view of ShipChamber's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'ShipChamber — no active sessions';
  const bits = [];
  ...
  return `ShipChamber — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show ShipChamber', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit ShipChamber', click: () => onAction({ type: 'quit' }) },
      );
    }
