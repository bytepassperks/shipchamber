...
files:
  - url: ShipChamber-1.2.3-win-${architecture}.exe
...
const createFixture = ({ includeArm64 = true } = {}) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-latest-yml-'));
...
const environment = ({ artifacts, output }) => ({
  ...
  RUNNER_TEMP: output,
  GH_REPO: 'bytepassperks/shipchamber',
  SHIPCHAMBER_VERSION: '1.2.3',
});
