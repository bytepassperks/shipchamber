...
const createPayload = () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-payload-test-'));
  fs.writeFileSync(path.join(root, 'shipchamber.desktop'), [
    '[Desktop Entry]', 'Name=ShipChamber', 'Exec=AppRun --no-sandbox %U', 'Icon=shipchamber', 'StartupWMClass=shipchamber', '',
  ].join('\n'));
  writeElf(path.join(root, 'shipchamber'), 'x64');
...
test('reads supported ELF architectures', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-elf-test-'));
  try {
