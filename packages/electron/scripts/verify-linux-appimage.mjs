...
export const verifyExtractedPayload = ({
  ...
  const desktopPath = path.join(root, 'shipchamber.desktop');
  ...
  for (const entry of ['Name=ShipChamber', 'Icon=shipchamber', 'StartupWMClass=shipchamber']) {
  ...
  assertElfArchitecture(path.join(root, 'shipchamber'), targetArchitecture, 'Electron executable');
...
const findAppImage = (version, architecture) => {
  ...
  const expected = path.join(electronRoot, 'dist', `ShipChamber-${version}-linux-${suffix}.AppImage`);
...
const main = () => {
  ...
  const target = normalizeTargetArchitecture(process.env.SHIPCHAMBER_TARGET_ARCH || process.arch).node;
  ...
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-appimage-'));
  try {
