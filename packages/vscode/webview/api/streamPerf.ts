...
declare global {
  interface Window {
    __shipchamberVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__shipchamberVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__shipchamberVsCodeStreamPerfState__ = {
  ...
  return window.__shipchamberVsCodeStreamPerfState__;
};
