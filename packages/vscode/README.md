...
![VS Code Extension](https://github.com/bytepassperks/shipchamber/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/bytepassperks/shipchamber) with even more features.**
...
Plus everything from the shared ShipChamber UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `ShipChamber: Focus Chat` | Focus the chat panel |
| `ShipChamber: New Session` | Start a new chat session |
| `ShipChamber: Open Sidebar` | Open the ShipChamber sidebar |
| `ShipChamber: Open Agent Manager` | Launch parallel multi-model runs |
| `ShipChamber: Open Session in Editor` | Open current or new session in an editor tab |
| `ShipChamber: Settings` | Open extension settings |
| `ShipChamber: Restart API Connection` | Restart the OpenCode API process |
| `ShipChamber: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **ShipChamber** submenu:
...
|---------|---------|-------------|
| `shipchamber.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `shipchamber.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
- `SHIPCHAMBER_VSCODE_BIN=cursor bun run vscode:dev`
- `SHIPCHAMBER_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/shipchamber-*.vsix`
