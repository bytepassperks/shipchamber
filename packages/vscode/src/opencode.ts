...
function getManagerOutputChannel(): vscode.OutputChannel {
  if (!managerOutputChannel) {
    managerOutputChannel = vscode.window.createOutputChannel('ShipChamberManager');
  }
...
function readShipChamberSettings(): Record<string, unknown> {
  const settingsPath = path.join(os.homedir(), '.config', 'shipchamber', 'settings.json');
  try {
...
function createConfiguredOpencodeBinaryError(raw: string, normalized: string): Error {
  const messageSuffix = 'ShipChamber needs the standalone opencode CLI. Install it and set shipchamber.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
...
function validateConfiguredOpencodeBinaryForManagedStart(): string | null {
  ...
  try {
    const settings = readShipChamberSettings();
...
function resolveOpencodeCliPath(): string | null {
  ...
  if (sharedFromShipChamber && isExecutable(sharedFromShipChamber) && !isKnownOpenCodeDesktopAppPath(sharedFromShipChamber)) {
    return sharedFromShipChamber;
  }
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    const onExit = (code: number | null) => {
      ...
      const appBundleHint = isMacOpenCodeAppBundlePath(binary)
        ? ' The configured binary appears to point at the macOS desktop app bundle; ShipChamber needs the standalone opencode CLI.'
        : '';
