...
const previousQuotaDataDirectory = process.env.SHIPCHAMBER_DATA_DIR;
const temporaryQuotaDataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-vscode-quota-'));
process.env.SHIPCHAMBER_DATA_DIR = temporaryQuotaDataDirectory;
...
after(() => {
  if (previousQuotaDataDirectory === undefined) delete process.env.SHIPCHAMBER_DATA_DIR;
  else process.env.SHIPCHAMBER_DATA_DIR = previousQuotaDataDirectory;
...
describe('OpenCode Go quota provider (VS Code parity)', () => {
  test('uses the opencode-go key from auth.json', async () => {
    ...
    assert.equal((request?.headers as Record<string, string>)['x-opencode-session'], 'shipchamber-usage');
