...
const TRAY_ACTION_EVENT = 'shipchamber:tray-action';
...
type TraySnapshot = {
  ...
  // Active instance label (e.g. "Local ShipChamber" or a remote host name) so
...
// the existing `shipchamber:open-session` / `shipchamber:open-draft-session`
...
const isTrayPlatform = (): boolean => {
  ...
  const platform = (window as unknown as { __SHIPCHAMBER_PLATFORM__?: string }).__SHIPCHAMBER_PLATFORM__;
  return platform === 'darwin' || platform === 'win32' || platform === 'linux';
};

const isTrayEnabled = (): boolean =>
  typeof window !== 'undefined' && window.__SHIPCHAMBER_ELECTRON__?.trayEnabled !== false;
...
// the local origin shows as "Local ShipChamber"; a remote host shows its
// configured name. Async because the host config is read over IPC.
const resolveInstanceName = async (): Promise<string> => {
  try {
    if (isDesktopLocalOriginActive()) return 'Local ShipChamber';
    const localOrigin = (window as unknown as { __SHIPCHAMBER_LOCAL_ORIGIN__?: string }).__SHIPCHAMBER_LOCAL_ORIGIN__
      || window.location.origin;
    const runtimeApiBaseUrl = getRuntimeApiBaseUrl();
    if (runtimeApiBaseUrl && locationMatchesHost(runtimeApiBaseUrl, localOrigin)) return 'Local ShipChamber';
...
export const useTraySync = (): void => {
  ...
  React.useEffect(() => {
    ...
    const bridge = (window as unknown as { __SHIPCHAMBER_DESKTOP__?: DesktopBridgeGlobal }).__SHIPCHAMBER_DESKTOP__;
