  ...
  useKeybinds({
    ...
    open_session_list: () => {
      ...
      if (state.isSidebarOpen) {
        window.dispatchEvent(new CustomEvent('shipchamber:sidebar-session-search'));
        return;
      }
    ...
    cycle_theme: () => {
      if (readEmbeddedThemeSearchParams() !== null && window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'shipchamber:cycle-theme-request' }, window.location.origin);
        return;
      }
