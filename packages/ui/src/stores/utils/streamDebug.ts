...
const STREAM_PERF_STORAGE_KEY = 'shipchamber_stream_perf';
...
declare global {
    interface Window {
        __shipchamberStreamPerfState?: StreamPerfState;
        __shipchamberStreamPerformance?: {
            setEnabled: (enabled: boolean) => void;
            reset: () => void;
            getSnapshot: () => StreamPerfSnapshot;
        };
        __shipchamberVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__shipchamberStreamPerfState) {
        const startedAt = Date.now();
        window.__shipchamberStreamPerfState = {
    ...
    return window.__shipchamberStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__shipchamberStreamPerfState;
        delete window.__shipchamberVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__shipchamberVsCodeStreamPerfState) {
        window.__shipchamberVsCodeStreamPerfState = {
            ...window.__shipchamberVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__shipchamberStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__shipchamberVsCodeStreamPerfState;
    if (!streamPerfEnabled || !state) {
        return {
            enabled: false,
            startedAt: null,
...
export const streamPerfMark = (metric: string): void => {
    ...
    performance.mark(`shipchamber.${metric}`);
};
...
if (typeof window !== 'undefined') {
    window.__shipchamberStreamPerformance = {
