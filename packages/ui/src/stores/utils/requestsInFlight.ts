...
const STORAGE_KEY = 'shipchamber_requests_in_flight';
...
declare global {
    interface Window {
        __shipchamberRequestsInFlight__?: RequestsInFlightState;
    }
}
...
const recordStart = (id: number, startMs: number): void => {
    const state = window.__shipchamberRequestsInFlight__;
...
export const setRequestsInFlightTrackingEnabled = (enabled: boolean): void => {
    ...
    try {
        if (enabled) {
            ...
            window.__shipchamberRequestsInFlight__ = createState();
            installFetchTracker();
            startSampling();
            return;
        }
        ...
        stopSampling();
        uninstallFetchTracker();
        delete window.__shipchamberRequestsInFlight__;
    } catch {
