...
export const useOpenInAppsStore = create<OpenInAppsState>()((set, get) => ({
  ...
  initialize: () => {
    ...
    window.addEventListener('shipchamber:settings-synced', settingsHandler);
    window.addEventListener('shipchamber:app-ready', appReadyHandler);
    window.addEventListener('shipchamber:installed-apps-updated', updateHandler);

    const appReady = (window as unknown as { __shipchamberAppReady?: boolean }).__shipchamberAppReady;
    if (appReady) {
      void loadInstalledApps();
    }
