...
import { getWorktreeSetupWaitEnabled, saveWorktreeSetupCommands } from '@/lib/shipchamberConfig';
...
const registerCreatedSession = (session: Session, directory: string): Session => {
  ...
  useSessionUIStore.getState().markSessionAsShipChamberCreated(session.id);
