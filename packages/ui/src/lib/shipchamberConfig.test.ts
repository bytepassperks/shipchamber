...
const project = { id: 'shipchamber', path: '/workspace/shipchamber' };
...
} = await import('./shipchamberConfig');

const getConfigPath = (projectPath: string): string => (
  `${homeDirectory}/.config/shipchamber/projects/${createProjectIdFromPath(projectPath)}.json`
);
