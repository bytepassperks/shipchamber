...
const sessionWith = (linked: unknown): Session =>
  ({ metadata: { shipchamber: { linked_issues: linked } } } as unknown as Session);
...
describe('withLinkedIssue', () => {
  test('adds a link and preserves unrelated metadata', () => {
    const next = withLinkedIssue(
      { shipchamber: { kind: 'review' }, other: 1 },
      issue(),
      true,
    );
    expect(next.other).toBe(1);
    expect((next.shipchamber as Record<string, unknown>).kind).toBe('review');
    expect((next.shipchamber as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([issue()]);
  });
  ...
  test('re-linking replaces the entry rather than duplicating it', () => {
    ...
    const stored = (second.shipchamber as { linked_issues: LinkedIssue[] }).linked_issues;
  ...
  test('unlinking removes only the matching id', () => {
    ...
    const stored = (next.shipchamber as { linked_issues: LinkedIssue[] }).linked_issues;
    expect(stored).toEqual([other]);
  });

  test('unlinking something absent is a no-op, not an error', () => {
    const next = withLinkedIssue({}, issue(), false);
    expect((next.shipchamber as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([]);
  });

  test('does not carry malformed stored entries forward', () => {
    const next = withLinkedIssue(
      { shipchamber: { linked_issues: [{ id: 'broken' }] } },
      issue(),
      true,
    );
    expect((next.shipchamber as { linked_issues: LinkedIssue[] }).linked_issues).toEqual([issue()]);
