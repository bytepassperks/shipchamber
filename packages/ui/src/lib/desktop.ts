...
export type DesktopSettings = {
  ...
  // User-added skills catalogs (persisted to ~/.config/shipchamber/settings.json)
...
const getElectronRuntime = (): ElectronRuntimeGlobal | null => {
  ...
  return (window as unknown as { __SHIPCHAMBER_ELECTRON__?: ElectronRuntimeGlobal }).__SHIPCHAMBER_ELECTRON__ ?? null;
};

const getDesktopBridge = (): DesktopBridgeGlobal | null => {
  if (typeof window === 'undefined') return null;
  return (window as unknown as { __SHIPCHAMBER_DESKTOP__?: DesktopBridgeGlobal }).__SHIPCHAMBER_DESKTOP__ ?? null;
};
...
export const isDesktopLocalOriginActive = (): boolean => {
  ...
  const local = typeof window.__SHIPCHAMBER_LOCAL_ORIGIN__ === 'string' ? window.__SHIPCHAMBER_LOCAL_ORIGIN__ : '';
...
export const getDesktopHomeDirectory = async (): Promise<string | null> => {
  if (typeof window !== 'undefined') {
    const embedded = window.__SHIPCHAMBER_HOME__;
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && bridge?.listen) {
      unlisten = await bridge.listen('shipchamber:update-progress', (evt) => {
