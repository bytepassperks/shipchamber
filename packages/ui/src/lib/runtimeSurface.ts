...
const detectHostedSurface = (): HostedSurface => {
  ...
  const explicitSurface = window.__SHIPCHAMBER_SURFACE__;
...
export const resolveHostedSurface = (): HostedSurface => {
  ...
  if (typeof window !== 'undefined') {
    window.__SHIPCHAMBER_SURFACE__ = surface;
  }
  return surface;
};
...
export const watchHostedSurfaceViewport = (): (() => void) => {
  ...
  const handleResize = () => {
    ...
    timeout = setTimeout(() => {
      timeout = null;
      const stamped = window.__SHIPCHAMBER_SURFACE__;
      const desired: HostedSurface = isPhoneViewport() ? 'mobile' : 'desktop';
      if (stamped && stamped !== desired) {
        window.__SHIPCHAMBER_SURFACE__ = undefined;
