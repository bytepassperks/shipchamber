...
export const getContextObligatoryMessages = (
  ...
  if (!isRecord(shipchamber) || !Array.isArray(shipchamber.context_obligatory_messages)) return [];

  return shipchamber.context_obligatory_messages.filter((value): value is ContextObligatoryMessage =>
    isRecord(value)
...
export const withContextObligatoryMessage = (
  ...
  const current = Array.isArray(shipchamber.context_obligatory_messages)
    ? shipchamber.context_obligatory_messages.filter((value): value is ContextObligatoryMessage =>
