...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'shipchamber:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'shipchamber:heartbeat') {
    return;
  }

  if (envelope.type === 'shipchamber:agent-memory-changed') {
  ...
  if (envelope.type === 'shipchamber:session-created') {
    const properties = getEventProperties(envelope.properties);
  ...
  if (envelope.type === 'shipchamber:browser-control-request') {
    const properties = getEventProperties(envelope.properties);
  ...
  if (envelope.type !== 'shipchamber:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const canControlBrowser = typeof window !== 'undefined' && Boolean(window.__SHIPCHAMBER_ELECTRON__);
...
export const subscribeShipchamberEvents = (listener: Listener): (() => void) => {
