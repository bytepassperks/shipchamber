...
const ANNOTATION_ACTIVE_FLAG = '__shipchamberBrowserAnnotationActive';

/** Cancels an overlay left behind by an earlier session. Safe when none is active. */
export const ANNOTATION_TEARDOWN_SCRIPT = `(() => {
  try {
    var host = document.querySelector('[data-shipchamber-annotation]');
    if (host && host.parentNode) host.parentNode.removeChild(host);
  } catch (error) { /* page navigated away */ }
  try {
    var cursor = document.getElementById('shipchamber-annotation-cursor');
...
export const buildAnnotationOverlayScript = (
  ...
  var OVERLAY_ATTR = 'data-shipchamber-annotation';
  ...
  cursorStyle.id = 'shipchamber-annotation-cursor';
