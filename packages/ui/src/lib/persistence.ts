...
export const applyPersistedHomeDirectoryToWindow = (homeDirectory: string): void => {
  ...
  if (typeof window.__SHIPCHAMBER_HOME__ === 'string' && window.__SHIPCHAMBER_HOME__.length > 0) {
    return;
  }

  try {
    window.__SHIPCHAMBER_HOME__ = homeDirectory;
  } catch {
    /* read-only contextBridge property — leave preload-seeded value */
  }
};

const SETTINGS_MIRROR_INDEX_KEY = 'shipchamber.settingsMirror.v2.index';
const SETTINGS_MIRROR_KEY_PREFIX = 'shipchamber.settingsMirror.v2:';
...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('shipchamber.pwaName', normalized);
    } else {
      localStorage.removeItem('shipchamber.pwaName');
    }
  } else {
    localStorage.removeItem('shipchamber.pwaName');
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings, bootstrap: boolean, adoptTheme = bootstrap): void => {
  ...
  window.dispatchEvent(new CustomEvent<SettingsSyncedDetail>('shipchamber:settings-synced', {
...
const dispatchSettingsSaveState = (state: 'saving' | 'saved' | 'error'): void => {
  ...
  window.dispatchEvent(new CustomEvent<'saving' | 'saved' | 'error'>('shipchamber:settings-save-state', { detail: state }));
};
...
export const syncDesktopSettings = async (options?: { bootstrap?: boolean; adoptTheme?: boolean }): Promise<void> => {
  ...
  const applySettings = async (loadedSettings: DesktopSettings) => {
    ...
    // `shipchamber:files:auto-save-enabled`. Prefer the hydrated store value and
