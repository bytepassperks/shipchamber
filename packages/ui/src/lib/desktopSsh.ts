...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    shipchamberPassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteShipchamber)
    ? value.remoteShipchamber
    : isRecord(value.remote_shipchamber)
      ? value.remote_shipchamber
      : {};
  ...
  const shipchamberPassword = parseStoredSecret(authRaw.shipchamberPassword || authRaw.shipchamber_password);
  ...
  return {
    ...
    auth: {
      ...
      ...(shipchamberPassword ? { shipchamberPassword } : {}),
    },
    portForwards,
  };
};
...
export const listenDesktopSshStatus = async (
  ...
  const desktop = (window as unknown as { __SHIPCHAMBER_DESKTOP__?: DesktopBridgeGlobal }).__SHIPCHAMBER_DESKTOP__;
  const listen = desktop?.listen;
  if (typeof listen !== 'function') {
    return async () => {};
  }

  const unlisten = await listen('shipchamber:ssh-instance-status', (event) => {
