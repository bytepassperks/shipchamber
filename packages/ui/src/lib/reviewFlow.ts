...
const requestChatForceScrollBottom = (sessionId: string): void => {
  ...
  window.dispatchEvent(new CustomEvent('shipchamber:chat-force-scroll-bottom', {
    detail: { sessionId },
  }));
};
...
const createOrReuseReviewSession = async (originalSessionID: string, directory: string, expectedRuntimeKey?: string): Promise<Session> => {
  ...
  if (existingReviewID) {
    ...
    await patchSessionMetadata(originalSessionID, directory, (metadata) => {
      const next = { ...metadata };
      const shipchamber = next.shipchamber;
      if (shipchamber && typeof shipchamber === 'object' && !Array.isArray(shipchamber)) {
        const rest = { ...(shipchamber as Record<string, unknown>) };
