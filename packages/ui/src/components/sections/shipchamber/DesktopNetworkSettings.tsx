...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  const isMacDesktop = isLocalDesktop
    ...
    && window.__SHIPCHAMBER_PLATFORM__ === 'darwin';
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.shipchamber.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.shipchamber.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.shipchamber.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.shipchamber.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleMinimizeToTrayToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status) {
        throw new Error(t('settings.shipchamber.desktopNetwork.error.minimizeToTraySaveFailed'));
      }
      if (!status.supported) {
        throw new Error(t('settings.shipchamber.desktopNetwork.error.minimizeToTrayUnsupported'));
      }
      setMinimizeToTrayEnabled(status.enabled);
    } catch (cause) {
      setMinimizeToTrayEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.shipchamber.desktopNetwork.error.minimizeToTraySaveFailed'));
    } finally {
  ...
  const handleKeepAwakeToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.shipchamber.desktopNetwork.error.keepAwakeUnsupported'));
      }
      setKeepAwakeEnabled(status.enabled);
    } catch (cause) {
      setKeepAwakeEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.shipchamber.desktopNetwork.error.keepAwakeSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.shipchamber.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.shipchamber.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.shipchamber.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <SettingsSection title={t('settings.shipchamber.desktopNetwork.title')}>
      <div className="space-y-3">
          ...
          <div className={SETTINGS_OPTION_STACK_CLASS}>
            {launchAtLoginSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.shipchamber.desktopNetwork.field.launchAtLogin')}
                info={t('settings.shipchamber.desktopNetwork.field.launchAtLoginDescription')}
                ariaLabel={t('settings.shipchamber.desktopNetwork.field.launchAtLoginAria')}
              />
            ) : null}
            ...
            {isMacDesktop ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.shipchamber.desktopNetwork.field.macMenuBar')}
                info={t('settings.shipchamber.desktopNetwork.field.macMenuBarDescription')}
                ariaLabel={t('settings.shipchamber.desktopNetwork.field.macMenuBarAria')}
              />
            ) : null}
            ...
            {minimizeToTraySupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.shipchamber.desktopNetwork.field.minimizeToTray')}
                info={t('settings.shipchamber.desktopNetwork.field.minimizeToTrayDescription')}
                ariaLabel={t('settings.shipchamber.desktopNetwork.field.minimizeToTrayAria')}
              />
            ) : null}
            ...
            {keepAwakeSupported ? (
              <SettingsCheckboxRow
                ...
                label={t('settings.shipchamber.desktopNetwork.field.keepAwake')}
                info={t('settings.shipchamber.desktopNetwork.field.keepAwakeDescription')}
                ariaLabel={t('settings.shipchamber.desktopNetwork.field.keepAwakeAria')}
              />
            ) : null}
          </div>
        ) : null}
        ...
        <SettingsStackedField
          ...
          label={(
            <label htmlFor="desktop-ui-password">
              {t('settings.shipchamber.desktopPassword.field.password')}
            </label>
          )}
          info={t('settings.shipchamber.desktopPassword.field.passwordDescription')}
        >
          <Input
            ...
            placeholder={t('settings.shipchamber.desktopPassword.field.passwordPlaceholder')}
            disabled={isLoading || isSaving}
            required={draftValue}
          ...
          <Button
            ...
            aria-label={t(showPassword ? 'settings.shipchamber.desktopPassword.actions.hidePassword' : 'settings.shipchamber.desktopPassword.actions.showPassword')}
        ...
        <div className={SETTINGS_OPTION_STACK_CLASS}>
          <SettingsCheckboxRow
            ...
            disabled={isLoading || isSaving}
            label={t('settings.shipchamber.desktopNetwork.field.allowLanAccess')}
            info={t('settings.shipchamber.desktopNetwork.field.allowLanAccessDescription')}
            description={(
              <>
                <span className="block text-[var(--status-warning)]/85">
                  {t('settings.shipchamber.desktopNetwork.field.warning')}
                </span>
                {lanRequiresPassword || lanBlockedByMissingPassword ? (
                  <span className="block text-[var(--status-warning)]/85">
                    {t('settings.shipchamber.desktopNetwork.field.passwordRequiredWarning')}
                  </span>
                ) : null}
              </>
            )}
            ariaLabel={t('settings.shipchamber.desktopNetwork.field.allowLanAccessAria')}
          />
        </div>
        ...
        {lanUrl ? (
          <div className="typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.shipchamber.desktopNetwork.hint.openAfterRestart')
              : t('settings.shipchamber.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.shipchamber.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </div>
    </SettingsSection>
