...
import { ShipChamberToolsSettings } from './ShipChamberToolsSettings';
...
import type { ShipChamberSection } from './types';
...
interface ShipChamberPageProps {
    /** Which section to display. If undefined, shows all sections (mobile/legacy behavior) */
    section?: ShipChamberSection;
}

export const ShipChamberPage: React.FC<ShipChamberPageProps> = ({ section }) => {
    const { t } = useI18n();
    ...
    if (!section) {
        return (
            <SettingsPageLayout showSaveStatus className="shipchamber-page-body space-y-3 sm:space-y-6">
                <ShipChamberVisualSettings />
                <DefaultsSettings />
                {showDesktopNetworkSettings && <DesktopNetworkSettings />}
                {!isVSCode && <OpenCodeCliSettings />}
                {!isVSCode && <ShipChamberToolsSettings />}
    ...
    return (
        <SettingsPageLayout
            ...
            showSaveStatus
            className="shipchamber-page-body"
        >
...
const GeneralSectionContent: React.FC = () => {
    ...
    return (
        <>
            ...
            {!isVSCode && <OpenCodeCliSettings />}
            {!isVSCode && <ShipChamberToolsSettings />}
            <ShipChamberVisualSettings visibleSettings={[
                'fileEditorKeymap',
...
const VisualSectionContent: React.FC = () => {
    const isVSCode = isVSCodeRuntime();
    return <ShipChamberVisualSettings visibleSettings={[
        'theme',
