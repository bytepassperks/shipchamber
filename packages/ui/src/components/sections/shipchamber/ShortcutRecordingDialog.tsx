  ...
  return (
    ...
    >
      <DialogContent className="max-w-md" initialFocus={recordingRef} showCloseButton={false}>
        <DialogHeader>
          <DialogTitle>
            {action ? t('settings.shipchamber.keyboardShortcuts.dialog.title', { action: actionLabel(action) }) : ''}
          </DialogTitle>
          <DialogDescription>{t('settings.shipchamber.keyboardShortcuts.dialog.instructions')}</DialogDescription>
        </DialogHeader>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            {recording.chords.length === 0 && !recording.livePreview ? (
              <span className="typography-ui-label text-muted-foreground">
                {t('settings.shipchamber.keyboardShortcuts.dialog.recording')}
              </span>
            ) : null}
          </div>
        </div>
        ...
        {recording.settled && protectedConflict ? (
          <p className="typography-meta text-[var(--status-error)]">
            {t('settings.shipchamber.keyboardShortcuts.error.internalConflict')}
          </p>
        ) : recording.settled && prefixConflict ? (
          <p className="typography-meta text-[var(--status-error)]">
            {t('settings.shipchamber.keyboardShortcuts.error.prefixConflict', { action: actionLabel(prefixConflict.action) })}
          </p>
        ) : null}
        {recording.settled && exactConflict && !protectedConflict && !prefixConflict ? (
          <p className="typography-meta text-[var(--status-warning)]">
            {t('settings.shipchamber.keyboardShortcuts.error.exactConflict', { action: actionLabel(exactConflict.action) })}
          </p>
        ) : null}
        {recording.settled && contextualPrefixConflict && !protectedConflict && !prefixConflict ? (
          <p className="typography-meta text-[var(--status-warning)]">
            {t('settings.shipchamber.keyboardShortcuts.warning.contextualPrefix', {
        ...
        {recording.settled && combo && isRiskyBrowserShortcut(combo) ? (
          <p className="typography-meta text-[var(--status-warning)]">
            {t('settings.shipchamber.keyboardShortcuts.warning.riskyBrowserShortcut')}
          </p>
        ) : null}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={confirm}
          >
            {t('settings.shipchamber.keyboardShortcuts.actions.confirm')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
