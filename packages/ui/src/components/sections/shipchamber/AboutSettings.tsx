...
import { ShipChamberLogo } from '@/components/ui/ShipChamberLogo';
...
const GITHUB_URL = 'https://github.com/bytepassperks/shipchamber';
const WEBSITE_URL = 'https://shipchamber.com';
...
export const AboutSettings: React.FC<AboutSettingsProps> = ({ initialUpdateDialogOpen = false }) => {
  ...
  const [shipChamberVersion, setShipChamberVersion] = React.useState<string | null>(null);
  ...
  const currentVersion = shipChamberVersion || updateStore.info?.currentVersion || 'unknown';

  React.useEffect(() => {
    let cancelled = false;

    const loadShipChamberVersion = async () => {
      try {
        ...
        const data = await response.json().catch(() => null) as { shipchamberVersion?: unknown } | null;
        const version = typeof data?.shipchamberVersion === 'string' && data.shipchamberVersion.trim().length > 0
          ? data.shipchamberVersion.trim()
          : null;
        if (!cancelled) setShipChamberVersion(version);
      } catch {
        if (!cancelled) setShipChamberVersion(null);
      }
    };

    void loadShipChamberVersion();
    ...
    if (updateStore.checking) {
      ...
      const timer = setTimeout(() => {
        ...
        if (didInitiateCheck.current && !updateStore.available && !updateStore.error) {
          toast.success(t('settings.shipchamber.about.toast.latestVersion'));
    ...
    return (
      <div className="w-full space-y-6 pb-2">
        <div className="flex flex-col items-center text-center">
          <ShipChamberLogo width={72} height={72} />
          <h2 className={`mt-4 ${SETTINGS_BRAND_TITLE_CLASS}`}>ShipChamber</h2>
          <div className="mt-2 space-y-1 typography-ui text-muted-foreground">
            <p>{t('aboutDialog.shipChamberVersionLabel', { version: currentVersion })}</p>
            <p>{t('aboutDialog.openCodeVersionLabel', { version: openCodeVersion || t('settings.shipchamber.about.state.unknown') })}</p>
          </div>
          <InstanceServiceUrls />
        </div>
        ...
        <div className="flex justify-center">
          {!updateStore.available && !updateStore.error && (
            ...
            >
              ...
              {isChecking ? t('settings.shipchamber.about.state.checking') : t('settings.shipchamber.about.actions.checkForUpdates')}
            </Button>
          )}
          ...
          {!isChecking && updateStore.available && (
            ...
            >
              ...
              {t('settings.shipchamber.about.actions.updateToVersion', { version: updateStore.info?.version || '' })}
            </Button>
          )}
        </div>
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="flex items-center justify-center gap-5">
            ...
            <a
              ...
              className="flex items-center gap-1.5 typography-ui-label text-muted-foreground transition-colors hover:text-foreground"
            >
              <Icon name="global" className="size-5" />
              <span>Website</span>
            </a>
          </div>
        </div>
      ...
      <div className="rounded-lg bg-[var(--surface-elevated)]/70 overflow-hidden flex flex-col">
        <div className="flex flex-col @xl:flex-row @xl:items-center justify-between gap-4 px-4 py-3 border-b border-border/40">
          <div className="flex min-w-0 flex-col">
            <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.shipchamber.about.field.version')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{currentVersion}</span>
          </div>
          <div className="flex min-w-0 flex-col">
            <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.shipchamber.about.field.openCodeVersion')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{openCodeVersion || t('settings.shipchamber.about.state.unknown')}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {updateStore.checking && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="typography-meta">{t('settings.shipchamber.about.state.checking')}</span>
              </div>
            )}
            ...
            {!updateStore.checking && !updateStore.available && !updateStore.error && (
              <span className="typography-meta text-muted-foreground">{t('settings.shipchamber.about.state.upToDate')}</span>
            )}
            ...
            >
              {t('settings.shipchamber.about.actions.checkForUpdates')}
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 border-b border-border/40 px-4 py-3 @xl:flex-row @xl:items-center @xl:justify-between">
          <span className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.shipchamber.about.field.instanceUrls')}</span>
          <InstanceServiceUrls />
        </div>
