...
export const OpenCodeCliSettings: React.FC = () => {
  ...
  return (
    <SettingsSection title={t('settings.shipchamber.opencodeCli.title')}>
      <div className="space-y-0.5">
        <SettingsFieldRow
          settingsItem="sessions.opencode-binary"
          label={t('settings.shipchamber.opencodeCli.field.binaryPath')}
          info={(
            <>
              {t('settings.shipchamber.opencodeCli.tipPrefix')}
              {' '}
              <span className="font-mono">OPENCODE_BINARY</span>
              {' '}
              {t('settings.shipchamber.opencodeCli.tipMiddle')}
              {' '}
              <span className="font-mono">~/.config/shipchamber/settings.json</span>
              {'.'}
            </>
          )}
          alignEnd={false}
        ...
        >
          <Input
            ...
            placeholder={t('settings.shipchamber.opencodeCli.field.binaryPathPlaceholder')}
          ...
          <Button
            ...
            aria-label={t('settings.shipchamber.opencodeCli.actions.browseAria')}
            title={t('settings.shipchamber.opencodeCli.actions.browse')}
          >
        ...
        <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
          {!isWindowsArm64() && (
            <SettingsCheckboxRow
              ...
              label={t('settings.shipchamber.opencodeCli.field.showUpdateNotifications')}
              ariaLabel={t('settings.shipchamber.opencodeCli.field.showUpdateNotificationsAria')}
            />
          )}
