...
import type { ShipChamberSection } from '@/components/sections/shipchamber/types';
import { ShipChamberPage } from '@/components/sections/shipchamber/ShipChamberPage';
import { AboutSettings } from '@/components/sections/shipchamber/AboutSettings';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__shipchamberSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const isWindows = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __SHIPCHAMBER_PLATFORM__?: string }).__SHIPCHAMBER_PLATFORM__ === 'win32';
  }, []);
  const isLinux = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __SHIPCHAMBER_PLATFORM__?: string }).__SHIPCHAMBER_PLATFORM__ === 'linux';
  }, []);
  ...
  const shipChamberSectionBySlug: Partial<Record<SettingsPageSlug, ShipChamberSection>> = React.useMemo(() => ({
    general: 'general',
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
  ...
  const openSearchResult = React.useCallback((result: SettingsSearchResult) => {
    ...
    if (result.id === 'plugins.create' && typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('shipchamber:settings-open-plugin-add'));
      }, 50);
    }
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'voice':
      case 'tunnel': {
        const section = shipChamberSectionBySlug[slug] ?? 'visual';
        return <ShipChamberPage section={section} />;
      }
      case 'home':
      default:
        return null;
    }
  }, [shipChamberSectionBySlug, renderUnavailable, runtimeCtx, t]);
