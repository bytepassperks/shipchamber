...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null, savedProjectPlan = null, onNavigatedToChat }) => {
  ...
  const initializeNewShipChamberSession = useSessionUIStore((state) => state.initializeNewShipChamberSession);
  ...
  const MD_VIEWER_MODE_KEY = 'shipchamber:plan:md-viewer-mode';
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      const planSubstance = resolvedPath
        ...
        : [
            instructionsText,
            '',
            'The plan is not stored as a file in the repository and has no file path. Its full current contents follow below this note and are the source of truth for the plan. Where the instructions above refer to the plan file, treat the plan as stored in ShipChamber project knowledge (it is edited through the ShipChamber UI): propose plan revisions as plan text in the chat rather than editing a file.',
            '',
            content,
          ].join('\n');
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewShipChamberSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, content, createSession, currentProjectRef, initializeNewShipChamberSession, isManagedChatPlan, pendingPlanSend, resolvedPath, routeToChat, savedPlanProjectRef, sendMessage, sendPromptTitle, setCurrentSession]
  );
