...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'shipchamber:files:md-viewer-mode';
  const HTML_VIEWER_MODE_KEY = 'shipchamber:files:html-viewer-mode';
  const JSON_VIEWER_MODE_KEY = 'shipchamber:files:json-viewer-mode';
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('shipchamber:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    return () => {
      window.removeEventListener('shipchamber:file-viewer-preview-mode-changed', handleFileViewerModeChanged);
    };
  }, [openPaths]);
