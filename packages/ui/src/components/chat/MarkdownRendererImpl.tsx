...
const FILE_LINK_SELECTOR = '[data-shipchamber-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-shipchamber-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-shipchamber-block-paths-scanned';
...
// `data-shipchamber-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-shipchamber-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-shipchamber-file-link');
      candidate.removeAttribute('data-shipchamber-file-ref');
      candidate.removeAttribute('data-shipchamber-file-path');
    ...
    const annotateFileLinksInner = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-shipchamber-file-link', 'true');
          candidate.setAttribute('data-shipchamber-file-ref', latestRawCandidate);
          candidate.setAttribute('data-shipchamber-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-shipchamber-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-shipchamber-file-link') !== 'true') {
        return;
      }
