...
interface Item {
  name: string;
  source: 'shipchamber' | 'opencode' | 'skill';
  description?: string;
...
describe('mergeCommandAutocompleteItems', () => {
  ...
  test('deduplicates every pairwise source collision by executable precedence', () => {
    const builtIn: Item = { name: 'compact', source: 'shipchamber', isBuiltIn: true };
  ...
  test('keeps a case-distinct command when the built-in is disabled', () => {
    const builtIn: Item = { name: 'init', source: 'shipchamber', isBuiltIn: true };
  ...
  test('keeps first-seen ordering and unrelated commands', () => {
    const builtIns: Item[] = [{ name: 'undo', source: 'shipchamber' }];
    const commands: Item[] = [
