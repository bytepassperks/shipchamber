...
export const Header: React.FC = () => {
  ...
  const checkRemoteInstanceUpdate = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await runtimeFetch(`/api/shipchamber/update-check?${params.toString()}`, {
        method: 'GET',
  ...
  const currentSessionTitle = React.useMemo(() => {
    if (!currentSessionId) {
      return activeProjectLabel ?? 'ShipChamber';
    }
  ...
  useEffect(() => {
    ...
    void syncFullscreenState();
    window.addEventListener('shipchamber:window-resized', onResize);

    return () => {
      disposed = true;
      window.removeEventListener('shipchamber:window-resized', onResize);
    };
