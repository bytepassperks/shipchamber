...
under the managed Chats owner (`shipchamber:chats`), for plans opened from a
...
`shipchamber:agent-memory-changed`, because the agent writes mid-turn through
...
`shipchamber:project-notes-updated` / `shipchamber:project-plan-saved` on the
