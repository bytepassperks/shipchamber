...
// foreign platform's chrome, not ShipChamber's own status tokens, so the
...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({
  ...
  useEffect(() => {
    ...
    window.addEventListener('shipchamber:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('shipchamber:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
