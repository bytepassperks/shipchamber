/**
 * ShipChamber deep-link vocabulary — the single source of truth for the `shipchamber://`
...
const DEEP_LINK_SCHEME = 'shipchamber';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (shipchamber://session/<id>),
  // but be tolerant of authority-less forms (shipchamber:/session/<id>) where it lands in
...
/**
 * Parse a raw `shipchamber://…` string into a typed intent, or `null` if it isn't a
 * recognised ShipChamber deep link. Tolerant by design: unknown routes return `null`
