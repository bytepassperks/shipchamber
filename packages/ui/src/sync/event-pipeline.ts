...
const normalizeShipChamberSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "shipchamber:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `shipchamber-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedShipChamberStatus = normalizeShipChamberSessionStatus(payload)
  if (normalizedShipChamberStatus) {
    return normalizedShipChamberStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("shipchamber:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("shipchamber:system-resume", onSystemResume)
