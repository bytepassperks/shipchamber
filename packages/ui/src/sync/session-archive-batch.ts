 ...
 * the ShipChamber server to archive the whole batch next to OpenCode, so the
 * browser spends one request and reconciles once.
 *
 * The route is an ShipChamber capability, not an OpenCode one. Runtimes that do
...
export async function requestSessionArchiveBatch(
  ...
  try {
    response = await runtimeFetch('/api/shipchamber/sessions/archive', {
      method: 'POST',
