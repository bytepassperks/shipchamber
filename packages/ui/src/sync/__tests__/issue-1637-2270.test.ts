...
const markSessionAsShipChamberCreatedCalls: string[] = []
...
mock.module("../session-ui-store", () => ({
  useSessionUIStore: {
    getState: () => ({
      ...
      markSessionAsShipChamberCreated: (sessionId: string) => {
        markSessionAsShipChamberCreatedCalls.push(sessionId)
      },
    }),
  },
}))
...
beforeEach(() => {
  ...
  markSessionAsShipChamberCreatedCalls.length = 0
