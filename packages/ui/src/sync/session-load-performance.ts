const STORAGE_KEY = "shipchamber_session_load_perf"
...
declare global {
  interface Window {
    __shipchamberSessionLoadPerformance?: SessionLoadPerformanceState
  }
}
...
export function startSessionLoadPerformanceEvent(input: Omit<SessionLoadPerformanceEvent, "at" | "durationMs" | "outcome">) {
  ...
  return (
    ...
    const state = window.__shipchamberSessionLoadPerformance ?? { events: [] }
    ...
    window.__shipchamberSessionLoadPerformance = state
  }
}
...
const FIRST_VISIBLE_MARK = "shipchamber.chat.first_message_visible"
