...
import { registerShipChamberRoutes } from './lib/opencode/shipchamber-routes.js';
...
import { createShipChamberSessionService } from './lib/shipchamber-sessions/routes.js';
import { createScheduledTaskService } from './lib/scheduled-tasks/service.js';
import { createShipChamberControlService } from './lib/shipchamber-control/service.js';
import { ShipChamberControlError } from './lib/shipchamber-control/error.js';
...
const DESKTOP_NOTIFY_PREFIX = '[ShipChamberDesktopNotify] ';
const uiNotificationClients = new Set();
const uiNotificationWsClients = new Set();
const uiShipChamberEventClients = new Set();
...
const SSE_PATH_PREFIXES = [
  ...
  '/api/shipchamber/events',
  '/api/shipchamber/realtime-proxy/sse',
];

function shouldSkipCompression(req, res) {
  if (process.env.SHIPCHAMBER_RUNTIME === 'desktop') {
    return true;
  }
...
const shouldSkipApiCompression = () => {
  if (isEnvFlagEnabled(process.env.SHIPCHAMBER_SKIP_API_COMPRESSION)) return true;
  if (isEnvFlagEnabled(process.env.SHIPCHAMBER_COMPRESS_API)) return false;
  if (isEnvFlagDisabled(process.env.SHIPCHAMBER_COMPRESS_API)) return true;
  return process.env.SHIPCHAMBER_RUNTIME === 'desktop';
};

const SHIPCHAMBER_VERBOSE_REQUEST_LOGS = isEnvFlagEnabled(process.env.SHIPCHAMBER_VERBOSE_REQUEST_LOGS);
...
const SHIPCHAMBER_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'shipchamber');
const SHIPCHAMBER_USER_THEMES_DIR = path.join(SHIPCHAMBER_USER_CONFIG_ROOT, 'themes');
const SHIPCHAMBER_PROJECTS_CONFIG_DIR = path.join(SHIPCHAMBER_USER_CONFIG_ROOT, 'projects');
// SHIPCHAMBER_CHATS_DIR relocates managed chat worktrees — needed when the
// OpenCode server runs as a separate user that cannot traverse $HOME.
const SHIPCHAMBER_CHATS_DIR = process.env.SHIPCHAMBER_CHATS_DIR && process.env.SHIPCHAMBER_CHATS_DIR.trim()
  ? path.resolve(process.env.SHIPCHAMBER_CHATS_DIR.trim())
  : path.join(SHIPCHAMBER_USER_CONFIG_ROOT, 'chats');
...
const themeRuntime = createThemeRuntime({
  fsPromises,
  path,
  themesDir: SHIPCHAMBER_USER_THEMES_DIR,
...
const SHIPCHAMBER_DATA_DIR = process.env.SHIPCHAMBER_DATA_DIR
  ? path.resolve(process.env.SHIPCHAMBER_DATA_DIR)
  : path.join(os.homedir(), '.config', 'shipchamber');
const SETTINGS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'push-subscriptions.json');
const APNS_TOKENS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'apns-tokens.json');
const REMOTE_CLIENTS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'remote-clients.json');
const CLIENT_PAIRING_SESSIONS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'client-pairing-sessions.json');
const CLOUDFLARE_MANAGED_REMOTE_TUNNELS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'cloudflare-managed-remote-tunnels.json');
const CLOUDFLARE_LEGACY_NAMED_TUNNELS_FILE_PATH = path.join(SHIPCHAMBER_DATA_DIR, 'cloudflare-named-tunnels.json');
...
const projectConfigRuntime = createProjectConfigRuntime({
  fsPromises,
  path,
  projectsDirPath: SHIPCHAMBER_PROJECTS_CONFIG_DIR,
});
...
const agentMemoryRuntime = createAgentMemoryRuntime({
  ...
  path,
  projectsDirPath: SHIPCHAMBER_PROJECTS_CONFIG_DIR,
  userConfigRoot: SHIPCHAMBER_USER_CONFIG_ROOT,
});
...
const hmrStateRuntime = createHmrStateRuntime({
  ...
  os,
  processLike: process,
  stateKey: '__shipchamberHmrState',
});
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.SHIPCHAMBER_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = (() => {
  if (process.env.SHIPCHAMBER_DESKTOP_NOTIFY === 'true') {
    return true;
  }
  ...
  return /shipchamber-server/i.test(argv0) || /shipchamber-server/i.test(argv1);
})();
...
const ENV_CONFIGURED_API_PREFIX = normalizeApiPrefix(
  process.env.OPENCODE_API_PREFIX || process.env.SHIPCHAMBER_API_PREFIX || ''
);
...
const processForwardedEventPayload = (payload, emitSyntheticEvent) => {
  ...
  emitSyntheticEvent({
    type: 'shipchamber:session-status',
    properties: {
      sessionID: sessionId,
      status,
      timestamp: Date.now(),
  ...
  emitSyntheticEvent({
    type: 'shipchamber:session-activity',
    properties: {
      sessionId,
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiShipChamberEventClients) {
      try {
        writeSseEvent(client, {
          type: 'shipchamber:scheduled-task-ran',
          properties: {
        ...
        uiShipChamberEventClients.delete(client);
      }
    }
  },
  logger: console,
...
const emitSessionCreatedEvent = (event) => {
  for (const client of uiShipChamberEventClients) {
    try {
      writeSseEvent(client, {
        type: 'shipchamber:session-created',
        properties: {
...
const resolveMemoryProjectId = createMemoryProjectResolver({
  ...
  managedProjectRoots: [...new Set([path.join(SHIPCHAMBER_USER_CONFIG_ROOT, 'chats'), SHIPCHAMBER_CHATS_DIR])],
});
...
const emitAgentMemoryChangedEvent = (event) => {
  for (const client of uiShipChamberEventClients) {
    try {
      writeSseEvent(client, {
        type: 'shipchamber:agent-memory-changed',
        properties: {
          scope: event.scope,
          ...(event.projectId ? { projectId: event.projectId } : {}),
        },
...
const shipChamberSessionService = createShipChamberSessionService({
  readSettingsFromDiskMigrated,
  sanitizeProjects,
...
// Browser actions are published to whichever ShipChamber clients are connected;
...
const browserControlBroker = createBrowserControlBroker({
  ...
  emitRequest: (request) => {
    ...
    let delivered = 0;
    for (const client of uiShipChamberEventClients) {
      if (needsBrowserView && client.shipchamberBrowserCapable !== true) continue;
      try {
        writeSseEvent(client, {
          type: 'shipchamber:browser-control-request',
          properties: {
...
const shipChamberControlService = createShipChamberControlService({
  readSettingsFromDiskMigrated,
  sanitizeProjects,
  buildOpenCodeUrl,
  getOpenCodeAuthHeaders,
  ...
  agentMemoryActions: createAgentMemoryActions({
    agentMemoryRuntime,
    createError: (message, status) => new ShipChamberControlError(message, status),
...
async function main(options = {}) {
  ...
  const effectiveBindHost = host
    || (typeof process.env.SHIPCHAMBER_HOST === 'string' && process.env.SHIPCHAMBER_HOST.trim().length > 0
      ? process.env.SHIPCHAMBER_HOST.trim()
      : '127.0.0.1');
  agentToolRuntime = createAgentToolRuntime({
    ...
    dataDir: SHIPCHAMBER_DATA_DIR,
    env: process.env,
    executeAction: (...args) => shipChamberControlService.execute(...args),
    getActivePort: () => {
  ...
  const uiPassword = typeof options.uiPassword === 'string'
    ? options.uiPassword
    : (typeof process.env.SHIPCHAMBER_UI_PASSWORD === 'string' ? process.env.SHIPCHAMBER_UI_PASSWORD : null);
  if (
  ...
  const apiOnly = options.apiOnly === true || isEnvFlagEnabled(process.env.SHIPCHAMBER_API_ONLY);
  ...
  console.log(`Starting ShipChamber on port ${port === 0 ? 'auto' : port}`);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    process,
    shipchamberVersion: SHIPCHAMBER_VERSION,
    runtimeName: process.env.SHIPCHAMBER_RUNTIME || 'web',
    serverStartedAt,
    gracefulShutdown,
    getHealthSnapshot: () => {
    ...
    verboseRequestLogs: SHIPCHAMBER_VERBOSE_REQUEST_LOGS,
    uiPassword,
    tunnelAuthController,
    remoteClientAuthRuntime,
    ...
    getServerLabel: () => {
      try {
        ...
        return typeof name === 'string' && name.trim().length > 0 ? name.trim() : 'ShipChamber';
      } catch {
        return 'ShipChamber';
      }
    },
    ...
    server,
    __dirname,
    shipchamberDataDir: SHIPCHAMBER_DATA_DIR,
  ...
  const relayService = createRelayService({
    ...
    hostLock: createRelayHostLock({
      lockFilePath: path.join(SHIPCHAMBER_DATA_DIR, 'relay-host.lock'),
      fs,
      process,
    }),
    ...
    // paired devices would land on them. SHIPCHAMBER_RELAY_HOST=off disables
    // passive hosting explicitly (dev scripts set it); the Electron dev shell is
    // covered via SHIPCHAMBER_ELECTRON_DEV. SHIPCHAMBER_RELAY_HOST=on overrides
    // both. Explicit enable/pairing on the instance still hosts regardless.
    allowPassiveHost: process.env.SHIPCHAMBER_RELAY_HOST === 'on'
      || (process.env.SHIPCHAMBER_RELAY_HOST !== 'off' && process.env.SHIPCHAMBER_ELECTRON_DEV !== '1'),
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    shipchamberDataDir: SHIPCHAMBER_DATA_DIR,
    shipchamberUserConfigRoot: SHIPCHAMBER_USER_CONFIG_ROOT,
    managedChatsRoot: SHIPCHAMBER_CHATS_DIR,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    getOpenCodeAuthHeaders,
    getOpenCodePort: () => openCodePort,
    // Dev-server discovery must not offer ShipChamber's own listeners back to
    ...
    waitForOpenCodeReady,
    emitSessionCreatedEvent,
    getShipChamberEventClients: () => uiShipChamberEventClients,
    writeSseEvent,
  ...
  const startupPipelineResult = await startupPipelineRuntime.run({
    ...
    attachSignals,
    apiOnly,
    dictationModelsDir: path.join(SHIPCHAMBER_USER_CONFIG_ROOT, 'speech-models'),
  });
  ...
  // Relay demand can change outside our routes: `shipchamber connect-url
