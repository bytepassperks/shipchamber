...
- This module owns GitHub auth, Octokit access, repo resolution, and Pull Request status resolution for ShipChamber.
...
- Auth storage: `~/.config/shipchamber/github-auth.json`
- Writes are atomic and file mode is `0o600`.
- Client ID resolution order: `SHIPCHAMBER_GITHUB_CLIENT_ID` -> `settings.json` -> default.
- Scope resolution order: `SHIPCHAMBER_GITHUB_SCOPES` -> `settings.json` -> default.
...
- PR state is persisted in local storage under `shipchamber.github-pr-status`.
