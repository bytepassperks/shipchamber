...
4. The **relay** (`shipchamber-website/apps/api`, Cloudflare Worker) verifies the signature +
...
- **Relay (default):** server has no Apple key; `SHIPCHAMBER_PUSH_RELAY_URL` defaults to
  `https://api.shipchamber.com/v1/push/send` (register URL is derived as `…/register-token`).
- **Direct (fallback):** set `SHIPCHAMBER_PUSH_RELAY_DISABLED=true` + `SHIPCHAMBER_APNS_KEY_ID/
  TEAM_ID/P8` to sign+send from the server itself (HTTP/2 + ES256 JWT); no relay binding needed.

## Config

Server (`apns-runtime.js`):
- `SHIPCHAMBER_PUSH_RELAY_URL` (default the public relay), `SHIPCHAMBER_APNS_ENVIRONMENT`
...
- Direct fallback: `SHIPCHAMBER_APNS_KEY_ID`, `SHIPCHAMBER_APNS_TEAM_ID`, `SHIPCHAMBER_APNS_P8`
  (or `_P8_PATH`), `SHIPCHAMBER_APNS_BUNDLE_ID`, `SHIPCHAMBER_PUSH_RELAY_DISABLED=true`.
...
1. Apple **Keys** (not Certificates) → create an **APNs Auth Key** (`.p8`) → Key ID + Team ID;
   enable **Push Notifications** on App ID `com.shipchamber.app`.
2. In the **shipchamber-website** repo → Actions secrets: `APNS_P8` (PEM), `APNS_KEY_ID`,
