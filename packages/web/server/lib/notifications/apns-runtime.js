...
//     for self-hosters who set SHIPCHAMBER_APNS_* and SHIPCHAMBER_PUSH_RELAY_DISABLED=true.
...
export const createApnsRuntime = (deps) => {
  ...
  const resolveApnsConfig = async () => {
    let keyId = trimmedEnv('SHIPCHAMBER_APNS_KEY_ID');
    let teamId = trimmedEnv('SHIPCHAMBER_APNS_TEAM_ID');
    let bundleId = trimmedEnv('SHIPCHAMBER_APNS_BUNDLE_ID');
    let environment = (trimmedEnv('SHIPCHAMBER_APNS_ENVIRONMENT') || '').toLowerCase();
    let p8 = normalizePem(process.env.SHIPCHAMBER_APNS_P8 || '');

    const p8Path = trimmedEnv('SHIPCHAMBER_APNS_P8_PATH');
    if (!p8 && p8Path) {
      try {
        p8 = (await fsPromises.readFile(p8Path, 'utf8')).trim();
      } catch (error) {
        console.warn('[APNs] Failed to read SHIPCHAMBER_APNS_P8_PATH:', error?.message ?? error);
      }
    }
  ...
  // is the fallback for self-hosters who set SHIPCHAMBER_APNS_* and disable the relay.
  const resolveRelayConfig = () => {
    if (trimmedEnv('SHIPCHAMBER_PUSH_RELAY_DISABLED') === 'true') return null;
    const url = trimmedEnv('SHIPCHAMBER_PUSH_RELAY_URL') || DEFAULT_RELAY_URL;
    const override = (trimmedEnv('SHIPCHAMBER_APNS_ENVIRONMENT') || '').toLowerCase();
    return {
      url,
      registerUrl: url.replace(/\/send$/, '/register-token'),
      // Explicit SHIPCHAMBER_APNS_ENVIRONMENT forces every send to that environment; when
  ...
  const sendViaRelay = async (deviceTokens, payload, relay, environment) => {
    ...
    const title = typeof payload?.title === 'string' && payload.title.length > 0 ? payload.title : 'ShipChamber';
    const { privateKey, publicJwk } = await getOrCreateRelayKeypair();
    const ts = Date.now();
  ...
  const sendViaDirectApns = async (tokenGroups, payload) => {
    ...
    if (!config) {
      if (!warnedUnconfigured) {
        ...
        console.warn(
          '[APNs] Relay disabled and no direct config; set SHIPCHAMBER_APNS_KEY_ID / SHIPCHAMBER_APNS_TEAM_ID / SHIPCHAMBER_APNS_P8 for direct send.',
        );
      }
      return;
    }
