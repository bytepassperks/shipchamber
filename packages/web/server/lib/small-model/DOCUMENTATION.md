...
plugins — this module replicates that mechanism as an ShipChamber runtime API.
...
- `resolve.js` — model selection, mirroring OpenCode's `getSmallModel` chain:
  0. ShipChamber's own settings override (Settings → Sessions → Small Model):
...
- `catalog.js` — models.dev catalog via the shared in-process cache
  (`../opencode/models-metadata.js`, also serving
   `/api/shipchamber/models-metadata`).
