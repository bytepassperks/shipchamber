...
describe('callSmallModel — custom provider config', () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    delete process.env.SHIPCHAMBER_TEST_PROVIDER_KEY;
    delete process.env.SHIPCHAMBER_TEST_GATEWAY_KEY;
  });
  ...
  describe('config-supplied credentials (no auth.json entry)', () => {
    ...
    it('resolves an OpenCode environment variable before sending the API key', async () => {
      process.env.SHIPCHAMBER_TEST_PROVIDER_KEY = 'sk-env-key';
      readConfig.mockReturnValue({
        provider: {
          custom: {
            options: { apiKey: '{env:SHIPCHAMBER_TEST_PROVIDER_KEY}', baseURL: 'https://proxy.example.test/v1' },
          },
        },
      });
      fetchMock.mockResolvedValue(ok('hello'));
    ...
    it('sends configured provider headers alongside the bearer token', async () => {
      process.env.SHIPCHAMBER_TEST_GATEWAY_KEY = 'sub-key';
      readConfig.mockReturnValue({
        provider: {
          custom: {
            options: {
              ...
              headers: {
                'Ocp-Apim-Subscription-Key': '{env:SHIPCHAMBER_TEST_GATEWAY_KEY}',
                'x-tenant': 'team',
              },
            },
          },
