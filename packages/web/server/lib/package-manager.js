...
const PACKAGE_NAME = '@shipchamber/web';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const GITHUB_RELEASES_URL = 'https://github.com/bytepassperks/shipchamber/releases';
const GITHUB_RELEASES_API_URL = 'https://api.github.com/repos/bytepassperks/shipchamber/releases';
...
function getShipChamberConfigDir() {
  if (process.platform === 'win32') {
    ...
    if (appData) return path.join(appData, 'shipchamber');
  }

  return path.join(os.homedir(), '.config', 'shipchamber');
}
  ...
  try {
    const response = await fetch(`${GITHUB_RELEASES_API_URL}/tags/v${version}`, {
      headers: {
        ...
        'User-Agent': 'shipchamber-update-check',
      },
    ...
    const apkAssets = Array.isArray(release?.assets)
      ...
      : [];
    const canonicalAsset = apkAssets.find((asset) => /^ShipChamber-.+-android\.apk$/i.test(asset.name));
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'shipchamber.cmd' : 'shipchamber';
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.SHIPCHAMBER_PACKAGE_MANAGER?.trim();
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('shipchamber');
  } catch {
    return false;
  }
}
  ...
  if (currentVersion !== 'unknown') {
    ...
    if (remote) {
      ...
      return {
        ...remote,
        packageManager: pm,
        updateCommand: 'shipchamber update',
      };
    }
  }
