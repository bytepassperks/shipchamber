...
// ---------------------------------------------------------------------------
// Regression for https://github.com/bytepassperks/shipchamber/issues/2746
//
// "[Bug] new worktree， Filename too long"
//
// ShipChamber places worktrees under:
...
const createTempDir = () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-git-issue2746-'));
  tempDirs.push(dir);
  return dir;
};
  ...
  it('enables core.longpaths and populates a deeply nested worktree checkout', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'shipchamber/issue-2746',
      });
  ...
  it('surfaces guided bootstrap failure when a path component exceeds the filesystem name limit', async () => {
    ...
    try {
      ...
      const created = await createWorktree(repo, {
        ...
        branchName: 'shipchamber/issue-2746-namemax',
      });
      expect(created.directoryCreated).toBe(true);
