...
const previousDataDir = process.env.SHIPCHAMBER_DATA_DIR;
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-quota-store-'));
process.env.SHIPCHAMBER_DATA_DIR = temporaryDirectory;
...
afterAll(() => {
  if (previousDataDir === undefined) delete process.env.SHIPCHAMBER_DATA_DIR;
  else process.env.SHIPCHAMBER_DATA_DIR = previousDataDir;
