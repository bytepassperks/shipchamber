...
const makeTempDir = () => fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-linear-routes-'));
...
describe('Linear auth routes', () => {
  ...
  it('starts authorization and completes it from the public callback', async () => {
    ...
    expect(callback.text).toContain('shipchamber://focus/linear-auth');
    ...
    expect(status.body.organization).toEqual({ id: 'org-1', name: 'ShipChamber', urlKey: 'shipchamber' });
  ...
  it('never exchanges a code whose state is unknown', async () => {
    ...
    expect(response.text).not.toContain('shipchamber://');
  });
  ...
  it('saves and reads Linear team-to-project mapping without leaking tokens', async () => {
    ...
    const saved = await request(app).put('/api/linear/mapping').send({
      defaultProjectPath: '/Users/ada/shipchamber',
    ...
    expect(reread.body.defaultProjectPath).toBe('/Users/ada/shipchamber');
