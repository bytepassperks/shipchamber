...
export async function startAuthorization({ origin } = {}) {
  ...
  if (!clientId) {
    throw new LinearOAuthError(
      'Linear OAuth client not configured. Set SHIPCHAMBER_LINEAR_CLIENT_ID.',
...
export async function consumeAuthorizationCallback({ code, state, error, errorDescription }) {
  ...
  if (!pending?.codeVerifier) {
    throw failAuthorization(
      'This authorization session has expired or is unknown to the running app. Return to ShipChamber and click Connect again.',
      'UNKNOWN_STATE',
    );
  }
