...
const DEFAULT_LINEAR_BROKER_URL = 'https://api.shipchamber.com/v1/oauth/linear';
...
function resolveDataDir() {
  const fromEnv = readEnv('SHIPCHAMBER_DATA_DIR');
  if (fromEnv) {
...
export function getLinearClientId() {
  const fromEnv = readEnv('SHIPCHAMBER_LINEAR_CLIENT_ID');
...
export function getLinearClientSecret() {
  const fromEnv = readEnv('SHIPCHAMBER_LINEAR_CLIENT_SECRET');
  if (fromEnv) return fromEnv;
  return readSettingString('linearClientSecret');
}

export function getLinearScopes() {
  const fromEnv = readEnv('SHIPCHAMBER_LINEAR_SCOPES');
  if (fromEnv) return fromEnv;
...
export function getLinearBrokerUrl() {
  const fromEnv = readEnv('SHIPCHAMBER_LINEAR_BROKER_URL');
...
export function getLinearRedirectUri() {
  const fromEnv = readEnv('SHIPCHAMBER_LINEAR_REDIRECT_URI');
  if (fromEnv) return fromEnv;
