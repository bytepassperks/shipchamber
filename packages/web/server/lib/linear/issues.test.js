...
const makeTempDir = () => fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-linear-issues-'));
...
describe('parseLinearIssueRef', () => {
  it('reads identifiers, URLs, and UUIDs', () => {
    ...
    expect(parseLinearIssueRef('https://linear.app/shipchamber/issue/ENG-12/broken-login'))
...
describe('Linear issue list/get', () => {
  ...
  it('searches by text and looks up an identifier directly', async () => {
    ...
    const byId = await listLinearIssues({ query: 'https://linear.app/shipchamber/issue/ENG-12' });
  ...
  it('creates a comment on the resolved issue UUID', async () => {
    vi.stubGlobal('fetch', vi.fn(async (_url, options) => {
      ...
      expect(body.variables.input).toEqual({
        ...
        body: 'ShipChamber session started.',
      });
      expect(options.headers.Authorization).toBe('Bearer access-1');
      return jsonResponse({
        data: {
