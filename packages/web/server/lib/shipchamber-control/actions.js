 ...
 */
export const SHIPCHAMBER_CONTROL_ACTION_DEFINITIONS = Object.freeze([
...
const SHIPCHAMBER_CONTROL_ACTIONS = Object.freeze(
  SHIPCHAMBER_CONTROL_ACTION_DEFINITIONS.map(({ action }) => action),
);

export const SHIPCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS = Object.freeze(
  SHIPCHAMBER_CONTROL_ACTION_DEFINITIONS.filter(({ agentExposed }) => agentExposed !== false),
);

export const SHIPCHAMBER_AGENT_TOOL_ACTIONS = Object.freeze(
  SHIPCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS.map(({ action }) => action),
);

export const SHIPCHAMBER_WEB_ACTION_DEFINITIONS = Object.freeze([
...
export const SHIPCHAMBER_WEB_ACTIONS = Object.freeze(
  SHIPCHAMBER_WEB_ACTION_DEFINITIONS.map(({ action }) => action),
);
 ...
 */
export const SHIPCHAMBER_MEMORY_ACTION_DEFINITIONS = Object.freeze([
...
export const SHIPCHAMBER_MEMORY_ACTIONS = Object.freeze(
  SHIPCHAMBER_MEMORY_ACTION_DEFINITIONS.map(({ action }) => action),
);
 ...
 * for `memory.read` from a tool already called `shipchamber_memory`, they send
...
const ACTIONS_BY_TOOL = Object.freeze({
  shipchamber: SHIPCHAMBER_AGENT_TOOL_ACTIONS,
  shipchamber_web: SHIPCHAMBER_WEB_ACTIONS,
  shipchamber_memory: SHIPCHAMBER_MEMORY_ACTIONS,
});
...
export const resolveAgentToolAction = (requested, toolName) => {
  ...
  const known = scoped ?? SHIPCHAMBER_ALL_ACTIONS;
  ...
  if (value) {
    const resolved = uniqueMatch(known, value)
      ...
      ?? (scoped ? null : uniqueMatch(SHIPCHAMBER_ALL_ACTIONS, value));
    if (resolved) {
  ...
  return {
    error: `Unsupported ShipChamber action: ${value || 'missing'}. Use one of: ${known.join(', ')}`,
  };
};

/** Everything the callback route will dispatch, whichever tool asked. */
export const SHIPCHAMBER_ALL_ACTIONS = Object.freeze([
  ...SHIPCHAMBER_CONTROL_ACTIONS,
