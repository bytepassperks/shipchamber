 ...
 * then `get` on `shipchamber_memory`, having dropped the namespace its own tool
...
describe('a namespace the tool name already implies', () => {
  test('resolves a bare action inside the calling tool', () => {
    expect(resolveAgentToolAction('read', 'shipchamber_memory')).toEqual({ action: 'memory.read' });
    expect(resolveAgentToolAction('save', 'shipchamber_memory')).toEqual({ action: 'memory.save' });
  });
  ...
  test('resolves a bare name that is ambiguous only across tools', () => {
    ...
    expect(resolveAgentToolAction('delete', 'shipchamber_memory')).toEqual({ action: 'memory.delete' });
    expect(resolveAgentToolAction('delete', 'shipchamber')).toEqual({ action: 'schedule.delete' });
  });

  test('keeps a fully qualified action as it is', () => {
    expect(resolveAgentToolAction('memory.read', 'shipchamber_memory')).toEqual({ action: 'memory.read' });
  });

  test('does not reach outside the tool that asked', () => {
    // The memory tool asking for `open` must fail, not drive the browser.
    expect(resolveAgentToolAction('open', 'shipchamber_memory').action).toBeUndefined();
  });
});
...
describe('what an unresolvable action reports', () => {
  test('names the actions the calling tool actually has', () => {
    const { error } = resolveAgentToolAction('get', 'shipchamber_memory');
  ...
  test('reports a missing action rather than resolving to something', () => {
    const { error, action } = resolveAgentToolAction('', 'shipchamber_memory');
  ...
  test('an unknown tool falls back to the full action list', () => {
    const { error } = resolveAgentToolAction('nonsense', 'shipchamber_future');
