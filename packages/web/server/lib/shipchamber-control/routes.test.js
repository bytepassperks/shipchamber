...
import { ShipChamberControlError } from './error.js';
import { registerShipChamberControlRoutes } from './routes.js';

const createApp = (execute) => {
  const app = express();
  registerShipChamberControlRoutes(app, { controlService: { execute } });
  return app;
};

describe('ShipChamber control route', () => {
  ...
  it('preserves service status and partial-result details', async () => {
    const execute = vi.fn(async () => {
      throw new ShipChamberControlError('dispatch failed', 500, {
        partial: true,
