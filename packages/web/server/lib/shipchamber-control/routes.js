...
export const registerShipChamberControlRoutes = (app, { controlService }) => {
  app.post('/api/shipchamber/control', express.json({ limit: '1mb' }), async (req, res) => {
    ...
    try {
      ...
      const controlError = asControlError(error, 'ShipChamber control action failed');
