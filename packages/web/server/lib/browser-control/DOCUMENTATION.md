...
- `../../index.js` supplies `emitRequest`, which writes the request to the
  ShipChamber SSE clients and returns how many were reached.
- `../shipchamber-control/service.js` is the only caller. It maps the
  `browser.*` actions of the `shipchamber_web` tool onto `broker.request()` and
