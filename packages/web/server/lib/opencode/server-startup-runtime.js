        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('ShipChamber startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'shipchamber:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`ShipChamber server listening on ${bindHost}:${activePort}`);
