...
//
// ShipChamber spawns the OpenCode server as an EXTERNAL child binary (on Unix
...
// whole process and is what caused the 1.13.3 `shipchamber-ui://` lag
...
const resolveRegistryDir = () => {
  const override = process.env.SHIPCHAMBER_MANAGED_PROCESS_REGISTRY;
  if (override && override.trim()) return override.trim();
  return path.join(os.homedir(), '.config', 'shipchamber', 'managed-opencode');
};
