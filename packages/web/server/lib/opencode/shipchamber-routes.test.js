...
const { registerShipChamberRoutes } = await import('./shipchamber-routes.js');
...
const createApp = ({ environment = {}, storedOptions = {} } = {}) => {
  ...
  const dependencies = {
    ...
    __dirname: '/opt/shipchamber/server',
    shipchamberDataDir: '/tmp/shipchamber',
  ...
  registerShipChamberRoutes(app, dependencies);
...
beforeEach(() => {
  ...
  packageManager.getUpdateCommand.mockReturnValue('npm install -g @shipchamber/web@latest');
});
...
describe('ShipChamber foreground update route', () => {
  ...
  it('rejects an unsafe systemd unit override before starting an update job', async () => {
    const { app } = createApp({
      environment: {
        ...
        SHIPCHAMBER_SYSTEMD_UNIT: 'shipchamber.service; rm -rf /',
      },
    });
  ...
  it('queues the install in a transient systemd unit and returns its job identifier', async () => {
    ...
    const { app } = createApp({
      environment: {
        INVOCATION_ID: 'systemd-invocation',
        SHIPCHAMBER_SYSTEMD_UNIT: 'shipchamber@wsl.service',
    ...
    await request(app)
      ...
      .expect(200, {
        ...
        jobId: 'shipchamber-update-1700000000000',
        logPath: 'journalctl --user-unit shipchamber-update-1700000000000.service',
      });

    expect(childProcess.spawnSync).toHaveBeenCalledWith('systemd-run', [
      '--user',
      '--unit=shipchamber-update-1700000000000',
      '--collect',
      '--service-type=exec',
      '--setenv=PATH=/home/syu/.npm-global/bin:/usr/bin:/bin',
      '/bin/sh',
      '-c',
      "set -eu\nnpm install -g @shipchamber/web@latest\nsystemctl --user restart 'shipchamber@wsl.service'",
    ], {
      encoding: 'utf8',
