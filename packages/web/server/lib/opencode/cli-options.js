export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.SHIPCHAMBER_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.SHIPCHAMBER_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.SHIPCHAMBER_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.SHIPCHAMBER_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.SHIPCHAMBER_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.SHIPCHAMBER_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.SHIPCHAMBER_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.SHIPCHAMBER_API_ONLY === '1' || env.SHIPCHAMBER_API_ONLY === 'true';
