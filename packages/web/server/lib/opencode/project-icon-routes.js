export const registerProjectIconRoutes = (app, dependencies) => {
  ...
  const projectIconsDirPath = path.join(shipchamberDataDir, 'project-icons');
  ...
  const applyProjectIconSvgTheme = (svgMarkup, themeVariant, iconColor) => {
    ...
    const overrideStyle = `<style data-shipchamber-theme-icon="1">:root{color:${color}!important;}</style>`;
