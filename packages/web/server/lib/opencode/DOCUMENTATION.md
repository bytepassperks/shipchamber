...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/ShipChamber route wiring.
...
- `packages/web/server/lib/opencode/shipchamber-routes.js`: ShipChamber update and models metadata route registration.
...
- Returned API:
  ...
  - `interruptBusySessionsAfterRestart()`: settles every session whose authoritative status is `busy`/`retry` or whose activity phase is still busy, broadcasts `shipchamber:session-status` idle plus an OpenCode-shaped `session.error`, resets leftover activity/cooldowns, and returns the interrupted session IDs in stable order.
  - `dispose()`
...
ShipChamber tool injection. Managed launch env strips AppImage `ARGV0` before
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside ShipChamber).
...
The pipeline binds the ShipChamber listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when ShipChamber binds port `0`.

## Public exports (shipchamber-routes.js)
- `registerShipChamberRoutes(app, dependencies)`: registers ShipChamber endpoints:
  - `GET /api/shipchamber/update-check`
  - `POST /api/shipchamber/update-install`
    - Foreground servers running under a systemd user unit queue installation in
      ...
      `SHIPCHAMBER_SYSTEMD_UNIT` overrides the default `shipchamber.service`.
  - `GET /api/shipchamber/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `SHIPCHAMBER_RATE_LIMIT_MAX_ATTEMPTS`, `SHIPCHAMBER_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
