...
const originalBundledOpencodeCliDir = process.env.SHIPCHAMBER_BUNDLED_OPENCODE_CLI_DIR;
const originalResourcesPath = process.resourcesPath;
const originalWslBinary = process.env.WSL_BINARY;
const originalShipChamberWslBinary = process.env.SHIPCHAMBER_WSL_BINARY;
...
afterEach(() => {
  ...
  if (typeof originalBundledOpencodeCliDir === 'string') {
    process.env.SHIPCHAMBER_BUNDLED_OPENCODE_CLI_DIR = originalBundledOpencodeCliDir;
  } else {
    delete process.env.SHIPCHAMBER_BUNDLED_OPENCODE_CLI_DIR;
  }
  ...
  if (typeof originalShipChamberWslBinary === 'string') {
    process.env.SHIPCHAMBER_WSL_BINARY = originalShipChamberWslBinary;
  } else {
    delete process.env.SHIPCHAMBER_WSL_BINARY;
  }
});
...
describe('OpenCode env runtime', () => {
  it('searches an explicit PATH without mutating the process environment', () => {
    const defaultDir = createTempDir('shipchamber-default-path-');
    const explicitDir = createTempDir('shipchamber-explicit-path-');
  ...
  it('clears AppImage ARGV0 when applying a login-shell env snapshot', () => {
    ...
    process.env.ARGV0 = '/path/to/ShipChamber.AppImage';
    delete process.env.SHIPCHAMBER_ARGV0_TEST_MARKER;
    const { runtime, state } = createRuntime({});
    state.cachedLoginShellEnvSnapshot = {
      PATH: '/usr/bin',
      ARGV0: '/leaked/from/shell.AppImage',
      SHIPCHAMBER_ARGV0_TEST_MARKER: '1',
    };

    try {
      runtime.applyLoginShellEnvSnapshot();
      expect(process.env.ARGV0).toBeUndefined();
      expect(process.env.SHIPCHAMBER_ARGV0_TEST_MARKER).toBe('1');
    } finally {
      delete process.env.SHIPCHAMBER_ARGV0_TEST_MARKER;
  ...
  it('throws a specific error for a configured directory without an executable CLI in strict mode', async () => {
    const dir = createTempDir('shipchamber-opencode-dir-');
  ...
  it('applies a valid configured executable OpenCode binary', async () => {
    const dir = createTempDir('shipchamber-opencode-bin-');
  ...
  it('keeps an env-provided OPENCODE_BINARY when the setting is an empty-string sentinel', async () => {
    const dir = createTempDir('shipchamber-env-opencode-');
    const binary = path.join(dir, 'opencode');
    fs.writeFileSync(binary, '#!/bin/sh\nexit 0\n');
    if (process.platform !== 'win32') fs.chmodSync(binary, 0o755);
  ...
  it('drops a previously applied settings override when the setting is cleared to an empty string', async () => {
    const dir = createTempDir('shipchamber-settings-opencode-');
    const binary = path.join(dir, 'opencode');
    fs.writeFileSync(binary, '#!/bin/sh\nexit 0\n');
    if (process.platform !== 'win32') fs.chmodSync(binary, 0o755);
  ...
  it('prefers the bundled CLI over a user-installed OpenCode from PATH', () => {
    const bundledDir = createTempDir('shipchamber-bundled-opencode-');
    const bundledBinary = path.join(bundledDir, process.platform === 'win32' ? 'opencode.exe' : 'opencode');
    const pathDir = createTempDir('shipchamber-path-opencode-');
    ...
    process.env.SHIPCHAMBER_BUNDLED_OPENCODE_CLI_DIR = bundledDir;
  ...
  it('keeps explicit OpenCode binary ahead of bundled CLI', () => {
    const bundledDir = createTempDir('shipchamber-bundled-opencode-');
    const bundledBinary = path.join(bundledDir, process.platform === 'win32' ? 'opencode.exe' : 'opencode');
    const explicitDir = createTempDir('shipchamber-explicit-opencode-');
  ...
  it('resolves the bundled OpenCode CLI from Electron resourcesPath', () => {
    const resourcesPath = createTempDir('shipchamber-resources-');
    ...
    process.env.PATH = createTempDir('shipchamber-empty-path-');
    delete process.env.SHIPCHAMBER_BUNDLED_OPENCODE_CLI_DIR;
    delete process.env.OPENCODE_BINARY;
    const emptyHome = createTempDir('shipchamber-empty-home-');
    const { runtime, state } = createRuntime({}, {
  ...
  it('rejects known Windows OpenCode desktop app install paths', async () => {
    setPlatform('win32');
    const localAppData = createTempDir('shipchamber-localappdata-');
  ...
  it('bounds every login-shell probe and falls through when one overruns', () => {
    ...
    const shellCalls = [];
    const { runtime } = createRuntime({}, {
      homedir: () => createTempDir('shipchamber-empty-home-'),
  ...
  it('does not auto-detect the Windows OpenCode desktop app as a CLI', () => {
    ...
    process.env.LOCALAPPDATA = localAppData;
    process.env.PATH = createTempDir('shipchamber-empty-path-');
    process.env.SystemRoot = createTempDir('shipchamber-empty-systemroot-');
    delete process.env.OPENCODE_BINARY;
    const { runtime } = createRuntime({}, {
      spawnSync: () => ({ status: 1, stdout: '', stderr: '' }),
    });
  ...
  it('skips Windows OpenCode desktop app entries returned by where.exe', () => {
    ...
    const localAppData = createTempDir('shipchamber-localappdata-');
    const desktopBinary = path.join(localAppData, 'Programs', 'OpenCode', 'OpenCode.exe');
    const cliBinary = path.join(createTempDir('shipchamber-cli-'), 'opencode.exe');
    fs.mkdirSync(path.dirname(desktopBinary), { recursive: true });
    fs.writeFileSync(desktopBinary, '');
  ...
  it('rejects WSL settings in strict mode', async () => {
    setPlatform('win32');
    const dir = createTempDir('shipchamber-no-wsl-');
    process.env.PATH = dir;
    process.env.SystemRoot = dir;
    process.env.WSL_BINARY = path.join(dir, 'missing-wsl.exe');
    process.env.SHIPCHAMBER_WSL_BINARY = path.join(dir, 'missing-shipchamber-wsl.exe');
  ...
  it('does not auto-detect OpenCode from WSL fallback paths', () => {
    setPlatform('win32');
    const dir = createTempDir('shipchamber-wsl-opencode-');
  ...
  it('launches Windows cmd shims through cmd call without embedded quotes', () => {
    ...
    const dir = createTempDir('shipchamber-opencode-cmd-');
  ...
  it('resolves npm OpenCode cmd shims to the packaged Windows executable', () => {
    setPlatform('win32');
    const npmDir = createTempDir('shipchamber-opencode-npm-');
