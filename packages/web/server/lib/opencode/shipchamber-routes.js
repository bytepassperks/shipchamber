...
function resolveSystemdServiceUnit(environment) {
  ...
  const configuredUnit = typeof environment.SHIPCHAMBER_SYSTEMD_UNIT === 'string'
    ? environment.SHIPCHAMBER_SYSTEMD_UNIT.trim()
    : '';
  const unit = configuredUnit || 'shipchamber.service';
...
export const registerShipChamberRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/shipchamber/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/shipchamber/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(shipchamberDataDir, 'run', `shipchamber-${currentPort}.json`);
      ...
      if (isForegroundService) {
        if (!systemdServiceUnit) {
          return res.status(409).json({
            error: 'Foreground servers must be updated by their service manager. Set SHIPCHAMBER_SYSTEMD_UNIT when running under systemd, or run shipchamber update and restart the service.',
          });
        }

        const updateJobName = `shipchamber-update-${Date.now()}`;
        ...
        return res.json({
          success: true,
          message: 'Update queued; ShipChamber will restart after installation completes',
          version: updateInfo.version,
          packageManager: pm,
          autoRestart: true,
      ...
      let restartCmdFallback = `shipchamber serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(shipchamberDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== ShipChamber update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting ShipChamber...
              ${restartCmd || 'echo Service manager will restart ShipChamber.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/shipchamber/models-metadata', async (_req, res) => {
    try {
