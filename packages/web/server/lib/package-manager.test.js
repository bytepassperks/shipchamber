...
describe('checkForUpdates', () => {
  ...
  it('resolves an Android APK asset when the update API returns an AAB', async () => {
    fetchMock
      .when('shipchamber-api.getlaunchpod.workers.dev', {
        ...
        json: async () => ({
          latestVersion: '1.10.0',
          updateAvailable: true,
          downloadUrl: 'https://github.com/bytepassperks/shipchamber/releases/download/v1.10.0/ShipChamber-1.10.0-42-android.aab',
        }),
      })
      .when('api.github.com/repos/bytepassperks/shipchamber/releases/tags/v1.10.0', {
        ok: true,
        json: async () => ({
          assets: [
            {
              name: 'ShipChamber-1.10.0-42-android.aab',
              browser_download_url: 'https://downloads.example/ShipChamber-1.10.0-42-android.aab',
            },
            {
            ...
            {
              name: 'ShipChamber-1.10.0-42-android.apk',
              browser_download_url: 'https://downloads.example/ShipChamber-1.10.0-42-android.apk',
            },
          ],
        }),
      });
    ...
    expect(result.downloadUrl).toBe('https://downloads.example/ShipChamber-1.10.0-42-android.apk');
  });
