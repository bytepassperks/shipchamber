...
objective stored under `metadata.shipchamber.goal`, with the small model as
an independent progress auditor. Built on ShipChamber's backend-driven
architecture (session-assist is the structural template): the loop lives in
the web server and survives UI disconnects.

## Goal payload (`metadata.shipchamber.goal`)
...
`SHIPCHAMBER_DATA_DIR` or `~/.config/shipchamber`), keyed by the SESSION ID:
...
- `routes.js` — `PUT/GET/DELETE /api/goals/objective/:sessionId`
  (ShipChamber-owned, registered before the generic proxy; JSON parsing via
...
stamp `metadata.shipchamber.goal` onto the fresh session (objective = the
...
`shipchamber session create --prompt <text> --goal` uses the explicit
`POST /api/shipchamber/sessions` orchestration route. The server creates the
...
`shipchamber session send --goal` and `shipchamber session fork --goal` use
