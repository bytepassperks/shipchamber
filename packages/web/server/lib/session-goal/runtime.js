...
// (metadata.shipchamber.goal). While the goal is active, the server keeps the
...
const parseGoalMetadata = (session) => {
  ...
  const namespace = metadata.shipchamber;
...
export const createSessionGoalRuntime = ({
  ...
  const writeGoal = async (sessionId, directory, expectedGoalId, mutate) => {
    ...
    await openCodeFetch(`/session/${encodeURIComponent(sessionId)}`, {
      ...
      body: {
        metadata: {
          ...currentMetadata,
          shipchamber: { ...currentNamespace, goal: nextGoal },
        },
      },
    });
    return nextGoal;
  ...
  const tick = async (sessionId, directory) => {
    ...
    // TEXT lives under the ShipChamber data dir keyed by session id and is
