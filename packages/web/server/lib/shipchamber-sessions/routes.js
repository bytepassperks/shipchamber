...
import { ShipChamberControlError, asControlError } from '../shipchamber-control/error.js';
...
const waitForWorktreeBootstrapReady = async ({ directory }) => {
  ...
  for (;;) {
    ...
    if (status?.status === 'failed') {
      throw new ShipChamberControlError(`Worktree bootstrap failed: ${status.error || 'unknown error'}`, 500);
    }
    const phase = status?.phase;
    if (status?.status === 'ready' || phase === 'git-ready' || phase === 'setup-ready') return;
    if (Date.now() >= deadline) {
      throw new ShipChamberControlError('Timed out waiting for the worktree bootstrap', 500);
    }
...
export const createShipChamberSessionService = (dependencies) => {
  const {
  ...
  const validateRequestedSelection = async ({ directory, requestedModel, requestedAgent, requestedVariant }) => {
    ...
    if (requestedAgent && agents.length > 0) {
      ...
      if (!agent) {
        throw new ShipChamberControlError(`Unknown agent '${requestedAgent}' for ${directory}`, 400);
      }
      if (!isPrimaryAgentMode(agent.mode)) {
        throw new ShipChamberControlError(`Agent '${requestedAgent}' is a subagent and cannot receive a prompt directly`, 400);
      }
    }

    if (requestedModel && providers.length > 0) {
      if (!hasProviderModel(providers, requestedModel.providerID, requestedModel.modelID)) {
        throw new ShipChamberControlError(
  ...
  const dispatchPrompt = async ({
    ...
    if (goalInput.enabled) {
      ...
      await (createSessionGoalOverride || createSessionGoal)({
        ...
        onWarning: (message, error) => console.warn(`[ShipChamberSessions] ${message}:`, error?.message || error),
      });
    }
  ...
  const archive = async (payload = {}) => {
    ...
    if (!parsed.ok) {
      throw new ShipChamberControlError(parsed.error, 400);
    }
    ...
    if (!resolvedDirectory.ok) {
      throw new ShipChamberControlError(resolvedDirectory.error, resolvedDirectory.status || 400);
    }
    ...
    for (const sessionID of parsed.ids) {
      try {
        ...
        console.warn('[ShipChamberSessions] failed to archive session', sessionID, error);
  ...
  const create = async (payload = {}) => {
    ...
    if (!goalInput.ok) {
      throw new ShipChamberControlError(goalInput.error, 400);
    }
    ...
    if (payload?.worktree && !worktreeInput) {
      throw new ShipChamberControlError('worktree.name is required when worktree is provided', 400);
    }
  ...
  const runExisting = async (action, sourceSessionId, payload = {}) => {
    ...
    const prompt = asNonEmptyString(payload.prompt);
    if (!sourceSessionID) throw new ShipChamberControlError('sessionId is required', 400);
    if (!prompt) throw new ShipChamberControlError('prompt is required', 400);
    const goalInput = resolveGoalInput(payload, prompt);
    if (!goalInput.ok) throw new ShipChamberControlError(goalInput.error, 400);
...
export const registerShipChamberSessionRoutes = (app, dependencies) => {
  const service = dependencies.sessionService || createShipChamberSessionService(dependencies);

  app.post('/api/shipchamber/sessions', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      return res.json(await service.create(req.body && typeof req.body === 'object' ? req.body : {}));
    } catch (error) {
      console.error('[ShipChamberSessions] failed to create session:', error);
      return sendServiceError(res, error, 'Failed to create session');
    }
  });

  app.post('/api/shipchamber/sessions/archive', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      return res.json(await service.archive(req.body && typeof req.body === 'object' ? req.body : {}));
    } catch (error) {
      console.error('[ShipChamberSessions] failed to archive sessions:', error);
      return sendServiceError(res, error, 'Failed to archive sessions');
    }
  });

  app.post(
    '/api/shipchamber/sessions/:sessionId/send',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.send(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[ShipChamberSessions] failed to send session:', error);
        return sendServiceError(res, error, 'Failed to send session');
      }
    },
  );
  app.post(
    '/api/shipchamber/sessions/:sessionId/fork',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.fork(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[ShipChamberSessions] failed to fork session:', error);
