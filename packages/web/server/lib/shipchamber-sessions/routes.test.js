...
globalThis.__shipchamberCreateWorktreeMock = createWorktreeMock;
globalThis.__shipchamberGetWorktreeBootstrapStatusMock = getWorktreeBootstrapStatusMock;

let registerShipChamberSessionRoutes;
...
vi.mock('../git/index.js', () => ({
  createWorktree: (...args) => globalThis.__shipchamberCreateWorktreeMock(...args),
  getWorktreeBootstrapStatus: (...args) => globalThis.__shipchamberGetWorktreeBootstrapStatusMock(...args),
}));
...
describe('shipchamber session routes', () => {
  beforeAll(async () => {
    ({ registerShipChamberSessionRoutes } = await import('./routes.js'));
  });
  ...
  describe('archiving a batch of sessions', () => {
    it('archives every id against the resolved directory and returns the archived sessions', async () => {
      ...
      const response = await request(app)
        .post('/api/shipchamber/sessions/archive')
    ...
    it('rejects an empty batch, an oversized batch, and non-string ids', async () => {
      ...
      await request(app).post('/api/shipchamber/sessions/archive').send({ directory: '/repo/app', ids: [] }).expect(400);
      await request(app)
        .post('/api/shipchamber/sessions/archive')
  ...
  it('creates a worktree before creating a session', async () => {
    ...
    try {
      ...
      const response = await request(app)
        ...
        .send({
          directory: '/repo/app',
          worktree: { name: 'side-task', branchName: 'shipchamber/side-task', startRef: 'main' },
          setUpstream: false,
          prompt: 'Run this',
          model: 'openai/gpt-5.5',
        })
  ...
  it('sends a goal prompt to an existing session after creating goal metadata', async () => {
    ...
    try {
      ...
      const { app } = createApp({ createSessionGoal });
      const response = await request(app)
        .post('/api/shipchamber/sessions/ses_source/send')
        .send({
          directory: '/repo/app',
  ...
  it('forks from a message, dispatches the prompt, and emits the new session', async () => {
    ...
    try {
      const { app } = createApp({ emitSessionCreatedEvent });
      const response = await request(app)
        .post('/api/shipchamber/sessions/ses_source/fork')
        .send({
          directory: '/repo/app',
