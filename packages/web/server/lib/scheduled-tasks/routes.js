...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/shipchamber/scheduled-tasks/status', async (_req, res) => {
    try {
  ...
  app.get('/api/shipchamber/events', (req, res) => {
    ...
    res.shipchamberBrowserCapable = req.query?.browser === '1';

    const clients = getShipChamberEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'shipchamber:event-stream-ready',
        properties: {
