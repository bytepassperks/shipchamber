...
Server-owned scheduled task runtime and routes for ShipChamber-only automation.
...
- This module is ShipChamber feature logic; it is intentionally separate from OpenCode proxy/runtime internals.

## Cross-instance occurrence claiming

Multiple ShipChamber server processes can share the same on-disk project config
...
- The sharing processes may run different ShipChamber versions. Normalization
...
- `packages/web/server/lib/scheduled-tasks/runtime.js`
  ...
  - Emits ShipChamber task-run events
...
- `packages/web/server/lib/scheduled-tasks/routes.js`
  ...
  - Manual run endpoint
  - ShipChamber events SSE stream endpoint
...
- Registers:
  ...
  - `GET /api/shipchamber/scheduled-tasks/status`
  - `GET /api/shipchamber/events`
