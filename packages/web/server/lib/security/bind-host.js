...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.SHIPCHAMBER_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `ShipChamber refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or SHIPCHAMBER_UI_PASSWORD before exposing it over LAN, '
  + 'or set SHIPCHAMBER_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';
