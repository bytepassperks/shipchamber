...
const DEFAULT_RP_NAME = 'ShipChamber';
...
const PASSKEY_STORE_FILE = path.join(SHIPCHAMBER_DATA_DIR, 'ui-passkeys.json');
...
export const createUiPasskeys = ({
  ...
  const beginRegistration = async (req, { label } = {}) => {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userID,
      userName: 'shipchamber-ui',
      userDisplayName: 'ShipChamber UI',
  ...
  const finishAuthentication = async (payload) => {
    ...
    if (!passkey) {
      const error = new Error('That passkey is not registered for this ShipChamber instance');
      error.statusCode = 404;
      throw error;
    }
