...
// client, and the /api/shipchamber/relay/* management routes.
//
...
// Routes are registered with the other ShipChamber feature routes, before the
...
const envRelayUrlOverride = () => {
  const raw = process.env.SHIPCHAMBER_RELAY_URL;
...
export const createRelayService = ({
  ...
  const readConfig = async () => {
    ...
    return {
      ...
      // True when the endpoint is pinned by SHIPCHAMBER_RELAY_URL (a self-hosted
  ...
  const standbyStatus = (holderPid) => ({
    state: 'standby',
    lastError: `relay host is owned by another local ShipChamber process (pid ${holderPid})`,
    connectedClients: 0,
  });
  ...
  const registerRoutes = (app) => {
    app.get('/api/shipchamber/relay/status', async (_req, res) => {
      try {
    ...
    app.post('/api/shipchamber/relay/disable', async (_req, res) => {
      try {
        const current = await readConfig();
