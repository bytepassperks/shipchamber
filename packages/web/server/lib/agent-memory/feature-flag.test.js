...
const original = process.env.SHIPCHAMBER_MEMORY_ENABLE;

afterEach(() => {
  if (original === undefined) delete process.env.SHIPCHAMBER_MEMORY_ENABLE;
  else process.env.SHIPCHAMBER_MEMORY_ENABLE = original;
});

describe('the unreleased feature gate', () => {
  test('is closed when the variable is unset', () => {
    delete process.env.SHIPCHAMBER_MEMORY_ENABLE;
    expect(isAgentMemoryFeatureAvailable()).toBe(false);
  });

  test('opens for the usual truthy spellings', () => {
    for (const value of ['1', 'true', 'TRUE', 'yes', 'on', ' true ']) {
      process.env.SHIPCHAMBER_MEMORY_ENABLE = value;
  ...
  test('is read per call, so a process started with it set is what decides', () => {
    delete process.env.SHIPCHAMBER_MEMORY_ENABLE;
    expect(isAgentMemoryFeatureAvailable()).toBe(false);
    process.env.SHIPCHAMBER_MEMORY_ENABLE = '1';
    expect(isAgentMemoryFeatureAvailable()).toBe(true);
  });
});
