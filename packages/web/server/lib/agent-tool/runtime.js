...
import {
  SHIPCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS,
  SHIPCHAMBER_AGENT_TOOL_ACTIONS,
  SHIPCHAMBER_MEMORY_ACTION_DEFINITIONS,
  SHIPCHAMBER_MEMORY_ACTIONS,
  resolveAgentToolAction,
  SHIPCHAMBER_WEB_ACTION_DEFINITIONS,
  SHIPCHAMBER_WEB_ACTIONS,
} from '../shipchamber-control/actions.js';
...
const ACTIONS = new Set([...SHIPCHAMBER_AGENT_TOOL_ACTIONS, ...SHIPCHAMBER_WEB_ACTIONS, ...SHIPCHAMBER_MEMORY_ACTIONS]);
...
const createToolEntry = ({ name, description, actions, definitions, parameters }) => String.raw`    ${name}: {
      ...
      args: {
        action: { type: "string", enum: ${JSON.stringify(actions)}, oneOf: ${JSON.stringify(definitions.map((entry) => ({ const: entry.action, description: entry.description })))}, description: "ShipChamber action to perform" },
      ...
      async execute(input, context) {
        ...
        const endpoint = process.env.SHIPCHAMBER_AGENT_TOOL_URL
        const token = process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN
        const failure = (payload) => ({
          title,
          output: JSON.stringify(payload),
          metadata: { shipchamber: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: false } },
        })
        if (!endpoint || !token) {
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "ShipChamber managed tool connection is unavailable" } })
        }
        ...
        try {
          ...
          if (valid) return { title, output, metadata: { shipchamber: { schemaVersion: ${TOOL_SCHEMA_VERSION}, action: args.action, description: title, ok: result.ok === true } } }
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "ShipChamber returned an invalid response", kind: "runtime", status: response.status } })
        } catch (error) {
...
const createPluginSource = ({ includeControl, includeWeb, includeMemory }) => {
  ...
  if (includeControl) {
    entries.push(createToolEntry({
      ...
      actions: SHIPCHAMBER_AGENT_TOOL_ACTIONS,
      definitions: SHIPCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS,
      parameters: CONTROL_PARAMETER_PROPERTIES,
    }));
  }
  if (includeWeb) {
    entries.push(createToolEntry({
      name: 'shipchamber_web',
      description: WEB_TOOL_DESCRIPTION,
      actions: SHIPCHAMBER_WEB_ACTIONS,
      definitions: SHIPCHAMBER_WEB_ACTION_DEFINITIONS,
      parameters: WEB_PARAMETER_PROPERTIES,
    }));
  }
  if (includeMemory) {
    entries.push(createToolEntry({
      name: 'shipchamber_memory',
      description: MEMORY_TOOL_DESCRIPTION,
      actions: SHIPCHAMBER_MEMORY_ACTIONS,
      definitions: SHIPCHAMBER_MEMORY_ACTION_DEFINITIONS,
      parameters: MEMORY_PARAMETER_PROPERTIES,
    }));
  }

  return `export const ShipChamberPlugin = async () => ({
  tool: {
${entries.join('')}  },
})
`;
...
export const createAgentToolRuntime = (dependencies) => {
  ...
  const pluginPath = path.join(pluginDirectory, 'shipchamber-plugin.js');
  let activeToken = null;

  const prepareManagedOpenCodeEnv = async ({ includeControl = true, includeWeb = true, includeMemory = true } = {}) => {
    const port = getActivePort();
    if (!Number.isInteger(port) || port <= 0) {
      throw new Error('ShipChamber listener port is unavailable for managed tool injection');
    }
    if (!includeControl && !includeWeb && !includeMemory) {
      throw new Error('At least one ShipChamber managed tool must be enabled to inject the plugin');
    }
    ...
    return {
      ...
      SHIPCHAMBER_AGENT_TOOL_URL: `http://127.0.0.1:${port}/api/shipchamber/agent-tool`,
      SHIPCHAMBER_AGENT_TOOL_TOKEN: activeToken,
    };
  };
  ...
  const execute = async (payload = {}, options = {}) => {
    ...
    if (!ACTIONS.has(action)) {
      return createResult({ ok: false, action, error: { message: `Unsupported ShipChamber action: ${action}`, kind: 'usage' } });
    }
    if (typeof executeAction !== 'function') {
      return createResult({ ok: false, action, error: { message: 'ShipChamber control service is unavailable', kind: 'runtime' } });
    }
    try {
  ...
  const registerRoutes = (app, express) => {
    app.post('/api/shipchamber/agent-tool', express.json({ limit: '1mb' }), async (req, res) => {
