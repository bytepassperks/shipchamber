...
import { SHIPCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS, SHIPCHAMBER_CONTROL_ACTION_DEFINITIONS } from '../shipchamber-control/actions.js';
...
const createRuntime = async (overrides = {}) => {
  const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'shipchamber-agent-tool-'));
...
describe('agent tool action allowlist', () => {
  it('defines a short title and agent description for every action', () => {
    expect(SHIPCHAMBER_CONTROL_ACTION_DEFINITIONS.every(({ action, title, description }) => action && title && description)).toBe(true);
  });
...
describe('managed agent tool runtime', () => {
  it('materializes the plugin and preserves configured plugin entries', async () => {
    ...
    const pluginPath = path.join(dataDir, 'agent-tool', 'shipchamber-plugin.js');
    ...
    expect(config.plugin).toEqual([
      ...
      expect.stringContaining('/agent-tool/shipchamber-plugin.js'),
    ]);
    expect(preparedEnv.SHIPCHAMBER_AGENT_TOOL_URL).toBe('http://127.0.0.1:3901/api/shipchamber/agent-tool');
    expect(preparedEnv.SHIPCHAMBER_AGENT_TOOL_TOKEN).toMatch(/^[A-Za-z0-9_-]+$/);
    expect(source).toContain('shipchamber: {');
    for (const { action, description } of SHIPCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS) {
    ...
    const hooks = await pluginModule.ShipChamberPlugin();
    expect(hooks.tool.shipchamber.description).toContain('Session dispatches return immediately by default');
    expect(hooks.tool.shipchamber.description).toContain('Set wait only when the user asks or the next step requires the completed result');
    expect(hooks.tool.shipchamber.args.action.oneOf).toContainEqual({
      const: 'session.messages',
      description: 'Read text-only messages and current sessionStatus for sessionId; directory and limit 10 are defaults',
    });
    expect(hooks.tool.shipchamber.args.parameters.properties.wait.description).toBe(
    ...
    expect(hooks.tool.shipchamber.args.parameters.properties.sessionId).toEqual({ type: 'string' });
    expect(source).not.toContain('title: "ShipChamber"');
    expect(source).not.toContain('@opencode-ai/plugin');
    expect(source).not.toContain(preparedEnv.SHIPCHAMBER_AGENT_TOOL_TOKEN);
  });
  ...
  it('emits both tools, each carrying only its own actions and inputs', async () => {
    ...
    const { tool } = await pluginModule.ShipChamberPlugin();

    const controlActions = tool.shipchamber.args.action.enum;
    const webActions = tool.shipchamber_web.args.action.enum;
    ...
    expect(Object.keys(tool.shipchamber_web.args.parameters.properties)).toContain('url');
    expect(Object.keys(tool.shipchamber.args.parameters.properties)).not.toContain('url');
    expect(Object.keys(tool.shipchamber.args.parameters.properties)).toContain('sessionId');
  });
  ...
  it('accepts inputs passed beside the action, not only inside parameters', async () => {
    ...
    const originalUrl = process.env.SHIPCHAMBER_AGENT_TOOL_URL;
    const originalToken = process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN;
    process.env.SHIPCHAMBER_AGENT_TOOL_URL = prepared.SHIPCHAMBER_AGENT_TOOL_URL;
    process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN = prepared.SHIPCHAMBER_AGENT_TOOL_TOKEN;
    ...
    try {
      ...
      await tool.shipchamber_web.execute(
      ...
      await tool.shipchamber.execute(
        { action: 'session.messages', sessionId: 'ses_1', limit: 3 },
        context,
      );
    } finally {
      globalThis.fetch = originalFetch;
      process.env.SHIPCHAMBER_AGENT_TOOL_URL = originalUrl;
      process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN = originalToken;
    }
  ...
  it('omits a tool the user turned off', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['shipchamber_web']);
  });
  ...
  it('exposes memory as its own tool carrying only its own inputs', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['shipchamber', 'shipchamber_memory']);
    expect(Object.keys(tool.shipchamber_memory.args.parameters.properties).sort())
    ...
    expect(Object.keys(tool.shipchamber.args.parameters.properties)).not.toContain('memoryId');
  });
  ...
  it('omits memory entirely when the user turns it off', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['shipchamber']);
  });
  ...
  it('injects the plugin when memory is the only tool left on', async () => {
    ...
    expect(Object.keys(tool)).toEqual(['shipchamber_memory']);
  });
  ...
  it('accepts the bare action a tool name already qualifies', async () => {
    // Observed: the model called `read` on shipchamber_memory, having taken the
  ...
  it('forwards cancellation to the shared control service', async () => {
    const executeAction = vi.fn(async (_action, _input, _directory, options) => {
      await new Promise((resolve, reject) => {
        options.signal.addEventListener('abort', () => reject(Object.assign(new Error('ShipChamber action was cancelled'), { statusCode: 499 })), { once: true });
      });
    });
    const { runtime } = await createRuntime({ executeAction });
    ...
    await expect(pending).resolves.toEqual(expect.objectContaining({
      ok: false,
      action: 'projects.list',
      error: { message: 'ShipChamber action was cancelled', kind: 'runtime' },
    }));
  ...
  it('requires the per-child token on the loopback route', async () => {
    ...
    await request(app)
      .post('/api/shipchamber/agent-tool')
      .send({ input: { action: 'projects.list' } })
      .expect(401);

    const response = await request(app)
      .post('/api/shipchamber/agent-tool')
      .set('authorization', `Bearer ${env.SHIPCHAMBER_AGENT_TOOL_TOKEN}`)
      .send({ input: { action: 'projects.list' } })
      .expect(200);
  ...
  it('executes through the materialized plugin and authenticated callback', async () => {
    ...
    const previousUrl = process.env.SHIPCHAMBER_AGENT_TOOL_URL;
    const previousToken = process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN;
    try {
      const env = await runtime.prepareManagedOpenCodeEnv();
      process.env.SHIPCHAMBER_AGENT_TOOL_URL = env.SHIPCHAMBER_AGENT_TOOL_URL;
      process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN = env.SHIPCHAMBER_AGENT_TOOL_TOKEN;
      const pluginPath = path.join(dataDir, 'agent-tool', 'shipchamber-plugin.js');
      const pluginModule = await import(`${pathToFileURL(pluginPath).href}?test=${Date.now()}`);
      const hooks = await pluginModule.ShipChamberPlugin();
      const metadata = vi.fn();

      const result = await hooks.tool.shipchamber.execute(
      ...
      expect(result.metadata.shipchamber.description).toBe('List configured projects');
      expect(metadata).toHaveBeenCalledWith(expect.objectContaining({
        title: 'List configured projects',
        metadata: expect.objectContaining({
          shipchamber: expect.objectContaining({ description: 'List configured projects' }),
        }),
      }));
    } finally {
      if (previousUrl === undefined) delete process.env.SHIPCHAMBER_AGENT_TOOL_URL;
      else process.env.SHIPCHAMBER_AGENT_TOOL_URL = previousUrl;
      if (previousToken === undefined) delete process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN;
      else process.env.SHIPCHAMBER_AGENT_TOOL_TOKEN = previousToken;
