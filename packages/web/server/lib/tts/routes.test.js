...
describe('normalizeCustomOpenAIBaseURL', () => {
  const originalRuntime = process.env.SHIPCHAMBER_RUNTIME;
  const originalAllowRemote = process.env.SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS;

  afterEach(() => {
    // Restore env vars after each test
    if (originalRuntime === undefined) {
      delete process.env.SHIPCHAMBER_RUNTIME;
    } else {
      process.env.SHIPCHAMBER_RUNTIME = originalRuntime;
    }
    if (originalAllowRemote === undefined) {
      delete process.env.SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
    } else {
      process.env.SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS = originalAllowRemote;
    }
  });

  it('rejects remote URLs when SHIPCHAMBER_RUNTIME is not set (web)', () => {
    delete process.env.SHIPCHAMBER_RUNTIME;
    delete process.env.SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  it('allows remote URLs when SHIPCHAMBER_RUNTIME is desktop', () => {
  ...
  it('allows remote URLs when SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS is true', () => {
    delete process.env.SHIPCHAMBER_RUNTIME;
    process.env.SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'true';
  ...
  it('denies remote URLs on desktop when env var is explicitly false', () => {
    process.env.SHIPCHAMBER_RUNTIME = 'desktop';
    process.env.SHIPCHAMBER_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'false';
