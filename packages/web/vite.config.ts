...
const pwaDevEnabled = process.env.SHIPCHAMBER_DISABLE_PWA_DEV !== '1';
...
const themeJsonHmrPlugin = () => ({
  name: 'shipchamber-theme-json-hmr',
  handleHotUpdate({ file, server }: { file: string; server: { ws: { send: (payload: unknown) => void } } }) {
    ...
    try {
      server.ws.send({
        type: 'custom',
        event: 'shipchamber:theme-updated',
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: '@shipchamber/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.SHIPCHAMBER_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.SHIPCHAMBER_PORT || 3001}`,
