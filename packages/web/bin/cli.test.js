...
async function withTempShipChamberDataDir(fn) {
  const previous = process.env.SHIPCHAMBER_DATA_DIR;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'shipchamber-cli-test-'));
  process.env.SHIPCHAMBER_DATA_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (typeof previous === 'string') {
      process.env.SHIPCHAMBER_DATA_DIR = previous;
    } else {
...
async function startMockShipChamberServer(options = {}) {
...
function spawnShipChamberLikeIdleProcess() {
  return spawn(process.execPath, ['-e', 'setInterval(() => {}, 1000)', 'shipchamber-idle'], { stdio: 'ignore' });
}

function spawnShipChamberLikeHungServer(port) {
  const script = `
    ...
    setInterval(() => {}, 1000);
  `;
  return spawn(process.execPath, ['-e', script, 'shipchamber-hung-server'], { stdio: 'ignore' });
}
...
describe('cli args', () => {
  ...
  it('parses explicit connect-url server overrides', () => {
    const parsed = parseArgs(['connect-url', '--server', 'https://shipchamber.example.com', '--port', '3002']);

    expect(parsed.command).toBe('connect-url');
    expect(parsed.options.server).toBe('https://shipchamber.example.com');
  ...
  it('parses session create options', () => {
    ...
    expect(parsed.options.branch).toBe('shipchamber/side-task');
  ...
  it('builds session create payloads from CLI options', () => {
    expect(buildSessionCreatePayload({
      ...
      worktree: {
        name: 'side-task',
        branchName: 'shipchamber/side-task',
        startRef: 'main',
      },
      prompt: 'Investigate cache invalidation',
      model: 'openai/gpt-5.5',
  ...
  it('formats projects compactly', () => {
    expect(formatProjectLine({
      id: 'path_repo',
      label: 'Shipchamber',
      path: '/repo/shipchamber',
    })).toBe('- `Shipchamber` — `path_repo` — `/repo/shipchamber`');
  });
...
describe('cli API target resolution', () => {
  ...
  it('fails when multiple non-default instances are running', async () => {
    await expect(resolveTargetPort({}, {
      ...
      isServerHealthReady: async () => false,
    })).rejects.toThrow('Multiple ShipChamber instances are running');
  });
});
...
describe('network-exposed auth validation', () => {
  ...
  it('allows explicit unsafe LAN override from process env only', () => {
    const previous = process.env.SHIPCHAMBER_ALLOW_UNAUTHENTICATED_LAN;
    process.env.SHIPCHAMBER_ALLOW_UNAUTHENTICATED_LAN = 'true';
    try {
      expect(() => assertAuthenticatedNetworkExposure({ host: '0.0.0.0' })).not.toThrow();
    } finally {
      if (typeof previous === 'string') {
        process.env.SHIPCHAMBER_ALLOW_UNAUTHENTICATED_LAN = previous;
      } else {
        delete process.env.SHIPCHAMBER_ALLOW_UNAUTHENTICATED_LAN;
      }
    }
  });
});
...
describe('serve host resolution', () => {
  it('uses SHIPCHAMBER_HOST when --host is not provided', () => {
    const previous = process.env.SHIPCHAMBER_HOST;
    process.env.SHIPCHAMBER_HOST = '192.0.2.20';
    try {
      expect(resolveServeHost(undefined)).toBe('192.0.2.20');
    } finally {
      if (typeof previous === 'string') {
        process.env.SHIPCHAMBER_HOST = previous;
      } else {
        delete process.env.SHIPCHAMBER_HOST;
      }
    }
  });

  it('prefers explicit --host over SHIPCHAMBER_HOST', () => {
    const previous = process.env.SHIPCHAMBER_HOST;
    process.env.SHIPCHAMBER_HOST = '192.0.2.20';
    try {
...
describe('compatibility exports', () => {
  it('allows tunnel profile migration before command options are initialized', async () => {
    await withTempShipChamberDataDir(async () => {
...
describe('CLI HTTP helpers', () => {
  it('sends one typed request to the shared control endpoint', async () => {
    ...
    globalThis.fetch = async (url, options = {}) => {
      expect(new URL(String(url)).pathname).toBe('/api/shipchamber/control');
  ...
  it('retries UI-authenticated API requests with the stored instance password', async () => {
    await withTempShipChamberDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/shipchamber/tunnel/start', {
          method: 'POST',
  ...
  it('prefers the stored instance password over a non-explicit env password', async () => {
    await withTempShipChamberDataDir(async () => {
      ...
      try {
        const { response, body } = await requestJson(port, '/api/shipchamber/scheduled-tasks/status', {
  ...
  it('authenticates desktop-local API requests with the stored client token', async () => {
    await withTempShipChamberDataDir(async (dir) => {
      const port = 57123;
...
describe('cli entry detection', () => {
  const modulePath = '/tmp/shipchamber/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/shipchamber';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/shipchamber';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'shipchamber')).toBe(true);
  });
...
describe('isShipchamberCmdline', () => {
  it('accepts ShipChamber CLI and daemon cmdlines', () => {
    expect(isShipchamberCmdline('node /x/@shipchamber/web/bin/cli.js serve')).toBe(true);
    expect(isShipchamberCmdline('node /x/@shipchamber/web/server/index.js --port 9090')).toBe(true);
    expect(isShipchamberCmdline('bun /home/u/projects/shipchamber/packages/web/server/index.js --port 3001')).toBe(true);
  });

  it('rejects recycled and unrelated processes (issue #1721)', () => {
    expect(isShipchamberCmdline('node /home/herjarsa/npm-global/bin/agentmemory')).toBe(false);
    expect(isShipchamberCmdline('node /usr/lib/node_modules/npm/bin/npm-cli.js install')).toBe(false);
    expect(isShipchamberCmdline('')).toBe(false);
    expect(isShipchamberCmdline(null)).toBe(false);
  });
});

describe('isShipchamberProcessRunning', () => {
  it('returns false for a dead PID', () => {
    expect(isShipchamberProcessRunning(2147483646)).toBe(false);
  });
  ...
  it.skipIf(process.platform !== 'linux' && process.platform !== 'darwin')(
    'returns false for a live non-ShipChamber PID',
    async () => {
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
        expect(isShipchamberProcessRunning(child.pid)).toBe(false);
      } finally {
        child.kill('SIGKILL');
      }
    }
...
describe('lifecycle instance discovery', () => {
  it('does not attribute a desktop runtime response to a different explicit port', async () => {
    await withTempShipChamberDataDir(async (dir) => {
      ...
      const instance = await discoverShipChamberInstanceOnPort(3003, {
  ...
  it('attributes a desktop runtime response to its configured desktop port', async () => {
    await withTempShipChamberDataDir(async (dir) => {
      ...
      const instance = await discoverShipChamberInstanceOnPort(57123, {
        fetchImpl: async () => createMockJsonResponse({ runtime: 'desktop', pid: 934 }),
      });
  ...
  it('keeps pid and instance files when live port probe confirms a cmdline mismatch', async () => {
    await withTempShipChamberDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        ...
        getShipchamberProcessState: () => 'mismatched',
      });
  ...
  it('preserves matched pid and instance files when the recorded port probe is inconclusive', async () => {
    await withTempShipChamberDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        fetchImpl: async () => createMockJsonResponse(null, false),
        getShipchamberProcessState: () => 'matched',
      });
  ...
  it('preserves unknown-identity pid and instance files when the recorded port probe is inconclusive', async () => {
    await withTempShipChamberDataDir(async () => {
      ...
      const instances = await discoverRunningInstances({
        fetchImpl: async () => createMockJsonResponse(null, false),
        getShipchamberProcessState: () => 'unknown',
      });
  ...
  it('uses the live system-info pid instead of a stale ShipChamber-looking pid-file pid', async () => {
    await withTempShipChamberDataDir(async () => {
      const port = 45127;
  ...
  it('discovers an explicit live ShipChamber port without a pid-file registry entry', async () => {
    await withTempShipChamberDataDir(async () => {
      const port = 45125;
      const instances = await discoverLifecycleInstances(
  ...
  it('cleans a matched pid-file entry without stopping it when the recorded port is free', async () => {
    await withTempShipChamberDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnShipChamberLikeIdleProcess();
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
        await new Promise((resolve) => setTimeout(resolve, 150));
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  it('serve refuses to start on a live ShipChamber port without requiring pid files', async () => {
    await withTempShipChamberDataDir(async () => {
      const server = await startMockShipChamberServer();
      try {
  ...
  it('stop --port can recover a matched pid-file instance whose HTTP endpoint is unresponsive', async () => {
    await withTempShipChamberDataDir(async () => {
      const port = await allocateLoopbackPort();
      const child = spawnShipChamberLikeHungServer(port);
      const pidFile = await getPidFilePath(port);
      const instanceFile = await getInstanceFilePath(port);
      try {
  ...
  it('plain stop ignores a stale CLI registry entry that resolves to desktop runtime', async () => {
    await withTempShipChamberDataDir(async () => {
      const server = await startMockShipChamberServer({ runtime: 'desktop' });
      const child = spawn('sleep', ['30'], { stdio: 'ignore' });
...
describe('Windows startup task command builder', () => {
  it('default-path length stays under 200 chars', () => {
    const cmd = buildWindowsStartupTaskCommand(
      'C:\\Users\\test\\.config\\shipchamber\\bin\\ShipChamber.ps1'
    );
  ...
  it('worst-case long path stays under 261-char Task Scheduler ceiling', () => {
    // Build a wrapper path >= 180 chars (simulates long SHIPCHAMBER_DATA_DIR)
    // Overhead = 57 chars (prefix + closing quote), so max wrapper for <261 total is 203
    const longPath =
      'C:\\Users\\' +
      'a'.repeat(139) +
      '\\.config\\shipchamber\\bin\\ShipChamber.ps1';
...
describe('startup command lingering output', () => {
  const linuxStatus = (lingerEnabled, lingerUser = 'alice') => ({
    ...
    active: true,
    activeState: 'active',
    servicePath: '/home/alice/.config/systemd/user/shipchamber.service',
    lingerEnabled,
    lingerUser,
  });
