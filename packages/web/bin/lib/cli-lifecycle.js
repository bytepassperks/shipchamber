...
function createLivePortInstance(port, info, host) {
  if (!hasShipchamberRuntimeInfo(info)) return null;
  return {
    port,
    pid: Number.isFinite(info.pid) ? info.pid : null,
    pidFilePath: path.join(getRunDir(), `shipchamber-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `shipchamber-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function fetchSystemInfoFromPortCandidates(port, fetchImpl, hosts, expectedPid) {
  for (const { host, requiresPidMatch } of hosts) {
    ...
    if (hasShipchamberRuntimeInfo(info)) {
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `shipchamber serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`shipchamber serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `shipchamber serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  const getProcessState = typeof options.getShipchamberProcessState === 'function'
    ? options.getShipchamberProcessState
    : (pid) => getShipchamberProcessState(pid, options);
  try {
    const files = fs.readdirSync(runDir);
    const pidFiles = files.filter((file) => file.startsWith('shipchamber-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('shipchamber-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `shipchamber-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `shipchamber-${port}.json`);
      ...
      // confirms ShipChamber. Cmdline identity alone can match a recycled PID
      // from another ShipChamber process on a different port. Try all plausible
      ...
      if (!hasShipchamberRuntimeInfo(liveInfo)) {
...
async function discoverShipChamberInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverShipChamberInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
...
async function discoverUnconfirmedRegistryInstanceOnPort(port, options = {}) {
  ...
  const storedOptions = readInstanceOptions(instanceFilePath);
  const processState = getShipchamberProcessState(pid);
  if (processState === 'dead') {
    removePidFile(pidFilePath);
    removeInstanceFile(instanceFilePath);
    return null;
