...
const STARTUP_SERVICE_ID = 'dev.shipchamber.web';
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'shipchamber.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'ShipChamber');
}

function getWindowsStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'ShipChamber.ps1');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (uiPassword) {
    env.SHIPCHAMBER_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.SHIPCHAMBER_API_ONLY = 'true';
  }
  if (typeof process.env.SHIPCHAMBER_DATA_DIR === 'string' && process.env.SHIPCHAMBER_DATA_DIR.trim().length > 0) {
    env.SHIPCHAMBER_DATA_DIR = path.resolve(process.env.SHIPCHAMBER_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'ShipChamber');
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'shipchamber.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'shipchamber.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'ShipChamber'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'shipchamber.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'shipchamber.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
