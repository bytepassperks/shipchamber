...
async function resolveTargetPort(options = {}, deps = {}) {
  ...
  if (ports.length > 1) {
    ...
    throw new TunnelCliError(
      `Multiple ShipChamber instances are running (ports: ${ports.join(', ')}). Choose one with --port <port>.`,
  ...
  throw new TunnelCliError(
    'No running ShipChamber server found. Start one with `shipchamber serve`, or pass --port <port>.',
