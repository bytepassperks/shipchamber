...
import { discoverShipChamberInstanceOnPort, isDesktopRuntimeForPort } from './cli-lifecycle.js';
...
async function serveCommand(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'ShipChamber serve' });
    }

    if (targetPort !== 0) {
      const existingInstance = await discoverShipChamberInstanceOnPort(targetPort, { host: effectiveHost });
      if (existingInstance?.runtime === 'desktop') {
        throw new Error(
          `Port ${targetPort} is used by ShipChamber Desktop app. Choose another port or stop the desktop app.`
        );
      }
      if (existingInstance) {
        const pidSuffix = Number.isFinite(existingInstance.pid) ? ` (PID: ${existingInstance.pid})` : '';
        if (existingInstance.source === 'probe') {
          throw new Error(`ShipChamber is already running on port ${targetPort}. Use \`shipchamber status\` or \`shipchamber stop --port ${targetPort}\`.`);
        }
        throw new Error(`ShipChamber is already running on port ${targetPort}${pidSuffix}`);
      }
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      ...
      const warningLine = 'SHIPCHAMBER_UI_PASSWORD is not set';
      const warningDetail = networkExposed
        ? `server is bound to ${bindHost} and reachable on your network with no UI auth. `
          + 'Set --ui-password or SHIPCHAMBER_UI_PASSWORD before exposing it over LAN.'
        : 'browser UI is unsecured. Use --ui-password or SHIPCHAMBER_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.SHIPCHAMBER_UI_PASSWORD = effectiveUiPassword;
      }
      process.env.SHIPCHAMBER_HOST = effectiveHost;
      process.env.SHIPCHAMBER_RUNTIME = 'web';
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting ShipChamber on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...process.env,
        SHIPCHAMBER_PORT: String(targetPort),
        SHIPCHAMBER_RUNTIME: 'web',
        OPENCODE_BINARY: opencodeBinary,
        SHIPCHAMBER_HOST: effectiveHost,
        ...(effectiveUiPassword ? { SHIPCHAMBER_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(options.apiOnly === true ? { SHIPCHAMBER_API_ONLY: 'true' } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { SHIPCHAMBER_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });

    child.unref();
    serveSpin?.start(`Starting ShipChamber on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`ShipChamber daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        }, DAEMON_READY_TIMEOUT_MS);

        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'shipchamber:ready' && typeof msg.port === 'number') {
            settled = true;
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`ShipChamber daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start ShipChamber');
    ...
    const serveResult = {
      ...
      logs: `shipchamber logs -p ${resolvedPort}`,
      launchMode: 'daemon',
    };
    ...
    if (isQuietMode(options)) {
      ...
      // in the same compact `pass:` token form `shipchamber status --quiet`
    ...
    if (!options.suppressStartupSummary && showOutput) {
      clackIntro('ShipChamber Started');
