...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `shipchamber-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `shipchamber-${port}.json`);
}
...
// that PID to an unrelated process — see isShipchamberProcessRunning.
...
function isShipchamberCmdline(cmdline) {
  if (typeof cmdline !== 'string' || cmdline.length === 0) {
    return false;
  }
  // Every install path contains the "shipchamber" segment — the npm package
  // (@shipchamber/web) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('shipchamber');
}

// Liveness + identity — "is the ShipChamber instance recorded in a pid file
...
function isShipchamberProcessRunning(pid) {
  const state = getShipchamberProcessState(pid);
  return state === 'matched' || state === 'unknown';
}

function getShipchamberProcessState(pid, options = {}) {
  ...
  return isShipchamberCmdline(cmdline) ? 'matched' : 'mismatched';
}

function hasShipchamberRuntimeInfo(info) {
