...
Rendering and hosting still happen in `shipchamber-website` (`apps/docs`).
...
- creates `shipchamber-docs-source-<sha>.tar.gz`
...
to `shipchamber-website` so it re-syncs and redeploys the docs. This fires on
...
- `SHIPCHAMBER_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `bytepassperks/shipchamber-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `shipchamber` (`main` on a
  push, the tag on a release)
- `archive_name`

`shipchamber-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`shipchamber` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `SHIPCHAMBER_REPO_TOKEN`
