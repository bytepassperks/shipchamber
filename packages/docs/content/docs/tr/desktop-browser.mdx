...
**Clear cookies** ve **Clear cached data** yalnızca bu panele uygulanır. ShipChamber oturumunuza ve diğer pencerelere dokunulmaz.
...
Ayrıca baktığı şeyin bir resmini proje içindeki `.shipchamber/screenshots/` klasörüne kaydedip yanıtında size gösterebilir. Öncesi ve sonrası karşılaştırmasını mümkün kılan şey budur ve dosya sonrasında bir pull request'e eklemek için orada kalır.

Tarayıcı eylemleri **ShipChamber Web tool**'dur. Bunu **Settings → General → ShipChamber Tools** altında ayrı ayrı açıp kapatabilirsiniz.
