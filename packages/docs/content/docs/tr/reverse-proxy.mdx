---
title: Reverse Proxy
description: ShipChamber'ı Nginx, Nginx Proxy Manager veya başka bir reverse proxy arkasında doğru şekilde ayarlayın.
---

# Reverse Proxy

ShipChamber'ı Nginx, Nginx Proxy Manager, Caddy, Cloudflare ya da başka bir reverse proxy arkasında çalıştırıyorsanız bu sayfayı kullanın.

## Proxy'lemeden önce

1. Önce ShipChamber'ın doğrudan çalıştığını doğrulayın.
...
- ShipChamber yanıtları zaten sıkıştırıyorsa proxy'de gzip'i kapatın.
...
- ShipChamber LAN üzerinden doğrudan erişilebilir
...
- ShipChamber HTTP yanıtlarını gzip ile sıkıştırır, eşik 1 KB'dir.
...
SSE akış rotaları ShipChamber tarafından sıkıştırmanın dışında tutulur, ancak CDN bunları yine de buffer'layabilir. SSE yollarında buffer'lamayı nasıl kapatacağınızı CDN belgelerinden kontrol edin.
