...
description: Tünelleri, uzak instance'ları ve ShipChamber'a başka bir cihazdan erişmeyi düzeltin.
---

# Uzaktan Erişim

ShipChamber'a telefonunuzdan ya da başka bir makineden ulaşamıyorsanız, çözüm bağlantı biçiminize göre değişir.
...
- sunucunun çalıştığını `shipchamber status` ile doğrulayın
...
- **Anywhere** eşleştirmesi için sunucudaki **Settings → Remote Instances → ShipChamber Relay** ayarını kontrol edin. **Connected** yazmalı. Değilse devre dışı bırakıp yeniden etkinleştirin
...
- `shipchamber tunnel status --all` çalıştırın
...
Bir [remote instance](/remote-instances/) takılırsa, ShipChamber hangi adımın başarısız olduğunu söyler:

- **auth** — SSH ya da UI parolanız reddedildi. Tekrar girin
- **install / start** — ShipChamber uzak makinede sunucuyu kuramadı ya da başlatamadı. O makinenin gereksinimlerini kontrol edin
- **forwarding** — bağlantı kuruludur ama port size ulaşmıyordur. Farklı bir local port deneyin

## Kendi sunucunuzun arkasında

ShipChamber'ı bir reverse proxy arkasına koyduysanız ve garip açılıyorsa ya da bağlanmıyorsa, [Reverse Proxy](/reverse-proxy/) bölümüne bakın.
