...
ShipChamber, yerel SSH yapılandırmanızdaki host'ları okuyabilir. Bu, `ssh work-server` gibi komutların kullandığı yerdir.
...
ShipChamber o SSH komutunu kullanarak bir uzak örnek oluşturur. **ready** noktasına geldiğinde uzak ShipChamber arayüzü masaüstü uygulamasında açılır.
...
ShipChamber üç yönlendirme tipini destekler:
...
Bilgisayarınızdaki diğer araçların uzak makine üzerinden gezmesini istiyorsanız **Dynamic (-D)** seçin. ShipChamber, o SSH bağlantısı için yerel bir SOCKS proxy portu açar.
...
- [Uzak örnekler](/remote-instances/) — masaüstü uygulamasını başka bir makinedeki ShipChamber'a bağlayın
