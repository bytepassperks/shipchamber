---
title: Reverse proxy
description: Skonfiguruj ShipChamber poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz ShipChamber za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że ShipChamber działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli ShipChamber już kompresuje odpowiedzi.
...
- ShipChamber dostępny bezpośrednio w LAN
...
- ShipChamber kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez ShipChamber, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
