---
title: Proxy reverso
description: Configure o ShipChamber corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o ShipChamber atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o ShipChamber funciona diretamente.
...
- Desative o gzip no proxy se o ShipChamber já comprime as respostas.
...
- ShipChamber acessível diretamente na LAN
...
- O ShipChamber comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo ShipChamber, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
