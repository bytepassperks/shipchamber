...
// Control Center control (iOS 18+): tap the ShipChamber logo to start a new session.
//
...
@available(iOS 18.0, *)
struct ShipChamberNewSessionControl: ControlWidget {
    var body: some ControlWidgetConfiguration {
        StaticControlConfiguration(kind: "ShipChamberNewSessionControl") {
        ...
        .description("Start a new ShipChamber session.")
    }
}

@available(iOS 18.0, *)
struct OpenNewSessionIntent: AppIntent {
    static let title: LocalizedStringResource = "New ShipChamber Session"
    static let openAppWhenRun: Bool = true
    static let isDiscoverable: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult & OpensIntent {
        return .result(opensIntent: OpenURLIntent(URL(string: "shipchamber://new")!))
    }
}
