...
enum WidgetStore {
    static let appGroup = "group.com.shipchamber.app"
...
enum WidgetDeepLink {
    static func newSession() -> URL { URL(string: "shipchamber://new")! }
    static func attention() -> URL { URL(string: "shipchamber://sessions?filter=attention")! }
    static func status() -> URL { URL(string: "shipchamber://status")! }
    static func settings() -> URL { URL(string: "shipchamber://settings")! }
    static func changes() -> URL { URL(string: "shipchamber://changes")! }
    static func files() -> URL { URL(string: "shipchamber://view/files")! }
    static func instances() -> URL { URL(string: "shipchamber://view/instances")! }
    static func session(_ id: String) -> URL {
        let encoded = id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id
        return URL(string: "shipchamber://session/\(encoded)") ?? newSession()
    }
}
...
// MARK: - Logo (full ShipChamber mark drawn from the SVG)

/// The ShipChamber logo, drawn to match packages/web/public/logo-dark-512x512.svg: an
