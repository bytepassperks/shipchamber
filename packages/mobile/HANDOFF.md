...
- App id / package: `com.shipchamber.app`; app name `ShipChamber`.
...
- **Deep links** — `shipchamber://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- **iOS widgets + Control Center + Notification Service Extension** — WidgetKit extension
  (`ShipChamberWidget`), a Control Center control, and an NSE (`ShipChamberNotificationService`)
  that refreshes widgets from push. All share the App Group `group.com.shipchamber.app`.
...
- Extensions: `ShipChamberWidget` (WidgetKit, deployment 17.0) and `ShipChamberNotificationService`
  (NSE, 15.5), both hand-wired into `App.xcodeproj/project.pbxproj` and embedded via a copy phase.
- App Group `group.com.shipchamber.app` in all three targets' entitlements (app + widget + NSE).
- `Info.plist`: `CFBundleURLTypes` scheme `shipchamber`, `NSCameraUsageDescription`.
- Push entitlement (aps-environment) required.
- APNs `mutable-content: 1` (set server/relay side) wakes the NSE to refresh widgets.

### Android (`android/app`)

- `google-services.json` (committed; Firebase project `shipchamber-8bf7e`). The Google Services
...
- Apple Developer account; App IDs for the app **and** both extensions
  (`com.shipchamber.app`, `.ShipChamberWidget`, `.ShipChamberNotificationService`), each enabled for
