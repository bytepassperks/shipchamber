#!/usr/bin/env node
/**
 * Fully automated streaming capture for ShipChamber.
 *
 ...
 * prompt through the supported `shipchamber session` CLI, and records until the
...
Records what ShipChamber costs while an assistant response streams in.

Options:
  --url <url>              ShipChamber URL (default: http://localhost:3000)
  --port <port>            ShipChamber CLI port (default: from --url)
...
/**
 * Runs an `shipchamber session` subcommand and returns its parsed JSON.
