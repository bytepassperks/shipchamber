#!/usr/bin/env node
/**
 * Fully automated session-switch latency capture for ShipChamber.
 *
...
Needs a running ShipChamber server; see scripts/perf/DOCUMENTATION.md.
`
...
const buildProbeSource = (sessionId) => `(() => {
  ...
  window.__shipchamberSwitchProbe = {
...
const readRenderCounters = async (client) => {
  const entries = await evaluateValue(client, `(window.__shipchamberStreamPerformance?.getSnapshot().entries ?? []).map((entry) => [entry.metric, entry.count])`)
  const counters = {}
...
const main = async () => {
  ...
  const profileDir = join(homedir(), ".cache", "shipchamber-perf-switch-profile")
  ...
  try {
    ...
    await client.send("Page.addScriptToEvaluateOnNewDocument", {
      source: `try { localStorage.setItem("shipchamber_stream_perf", "1") } catch {}`,
    })
    ...
    for (const [index, { id, visit }] of visits.entries()) {
      ...
      await evaluateValue(client, `window.__shipchamberSwitchProbe.start()`)
      await pressAt(client, box.x, box.y)
      await wait(options.gap)
      const probe = await evaluateValue(client, `window.__shipchamberSwitchProbe.finish()`)
