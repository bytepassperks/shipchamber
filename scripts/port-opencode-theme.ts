...
/*
 * Port OpenCode themes into full ShipChamber theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/shipchamber-theme-port-test --force
 ...
 *     or existing ShipChamber theme JSON files
 *   - writes complete ShipChamber light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | ShipChamberTheme;
...
function isShipChamberTheme(value: ThemeSource): value is ShipChamberTheme {
...
function syntheticTokensFromShipChamberTheme(theme: ShipChamberTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: ShipChamberTheme }> = [];
    ...
    if (isShipChamberTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromShipChamberTheme(source),
