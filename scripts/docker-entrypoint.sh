...
# alias; SHIPCHAMBER_UI_PASSWORD is the canonical runtime variable.
if [ -z "${SHIPCHAMBER_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  SHIPCHAMBER_UI_PASSWORD="$UI_PASSWORD"
  export SHIPCHAMBER_UI_PASSWORD
fi

if [ -n "${SHIPCHAMBER_UI_PASSWORD:-}" ]; then
...
SHIPCHAMBER_HOST="${SHIPCHAMBER_HOST:-0.0.0.0}"
export SHIPCHAMBER_HOST
...
if [ -n "${SHIPCHAMBER_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$SHIPCHAMBER_UI_PASSWORD"
fi
"$@"
