...
# Usage: curl -fsSL https://raw.githubusercontent.com/btriapitsyn/shipchamber/main/scripts/install.sh | bash

set -euo pipefail

PACKAGE_NAME="@shipchamber/web"
...
main() {
  ...
  # If ShipChamber is already installed, hand off to its own updater instead
  # of guessing a package manager. `shipchamber update` detects which manager
  ...
  if command_exists "$BIN_NAME"; then
    info "ShipChamber is already installed — updating via 'shipchamber update'..."
    echo ""
    if shipchamber update; then
      echo ""
      success "ShipChamber is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "ShipChamber installed successfully!"
    echo ""
    ...
    if command_exists "$BIN_NAME"; then
      echo "  Get started:"
      echo "    shipchamber              # Start server on port 3000"
      echo "    shipchamber --help       # Show all options"
    else
      ...
      if [ -n "$bin_dir" ]; then
        ...
        echo "  then restart your terminal and run: shipchamber"
      fi
    fi
    echo ""
    echo "  Prerequisites:"
